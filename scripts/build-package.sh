...
if [ "${SKIP_DOCKER:-0}" != "1" ]; then
    ...
    BUILD_ID="$(date -u +%Y%m%dT%H%M%S)-$(git -C "${WS}/ussdgateway" rev-parse --short HEAD 2>/dev/null || echo local)"
...
EOF
    echo "  manifest BUILD_ID=${BUILD_ID} (active=${USSDGW_IMAGE_VARIANT})"
else
