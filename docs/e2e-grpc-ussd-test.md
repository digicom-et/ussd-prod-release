...
| Cách | Ai dùng | Độ khó |
|------|---------|--------|
| **[A] Package `ussdgw-prod-release`** | Đem lên server production, giải nén và chạy | ⭐ Dễ — **đọc phần này trước** |
| **[B] Dev machine** | Build từ source `ussdgateway` + `jSS7` | ⭐⭐⭐ Khó hơn — cuối tài liệu |

> **Package offline:** Bundle sẵn tại `ussdgw-prod-release/` — xem [`ussdgw-prod-release/README.md`](../../ussdgw-prod-release/README.md).
...
- Biến môi trường chung: `source ./scripts/env.sh` (hoặc `PKG_ROOT=/opt/ussdgw-prod-release`).
...
# [A] Chạy bằng package `ussdgw-prod-release` (khuyến nghị)
...
- `python3` (3.9–3.12)
- RAM ≥ 6 GB
- File package đã giải nén, ví dụ: `/opt/ussdgw-prod-release/`
...
```bash
cd /opt/ussdgw-prod-release          # đổi path nếu ông chủ để chỗ khác
chmod +x scripts/*.sh
...
Giả sử `cd /opt/ussdgw-prod-release` và `source ./scripts/env.sh`.
...
**Phải thấy** container `ussd-prod` trạng thái `running` (hoặc `healthy` sau ~3–5 phút).
...
|---------|-----------|---------|
| `init` | `ussd-prod-init` | Chạy 1 lần: seed `/opt/ussdgw/data` |
...
| # | Lệnh | Làm gì |
|---|------|--------|
| 1 | `cd /opt/ussdgw-prod-release/gateway` | Vào thư mục compose |
...
| 5 | `docker logs -f ussd-prod` | Theo dõi deploy SLEE (tuỳ chọn) |
...
| # | Lệnh | Làm gì |
|---|------|--------|
| 1 | `cd /opt/ussdgw-prod-release/tools/grpc-as-tester` | Thư mục Python AS + `menu_config.json` |
...
Dừng AS: `kill $(cat /opt/ussdgw-prod-release/.grpc-as.pid)` hoặc `./scripts/05-stop-grpc-as.sh`.
...
| # | Lệnh | Làm gì |
|---|------|--------|
| 1 | `cd /opt/ussdgw-prod-release/tools/jss7-map-load` | MAP load client (JAR trong `lib/`) |
...
| # | Lệnh | Làm gì |
|---|------|--------|
| 1 | `cd /opt/ussdgw-prod-release/tools/grpc-as-tester` | Cùng venv với `ussd_as_server.py` |
...
| # | Lệnh | Làm gì |
|---|------|--------|
| 1 | `kill $(cat /opt/ussdgw-prod-release/.grpc-as.pid 2>/dev/null) 2>/dev/null; rm -f .grpc-as.pid` | Dừng gRPC AS |
| 2 | `kill $(cat /opt/ussdgw-prod-release/.http-as.pid 2>/dev/null) 2>/dev/null; rm -f .http-as.pid` | Dừng HTTP Pull AS (nếu có) |
| 3 | `cd /opt/ussdgw-prod-release/gateway && docker compose down` | Dừng gateway + init |
...
Bảng dưới map **1:1** từng script → lệnh tay. `PKG=/opt/ussdgw-prod-release` (đổi nếu giải nén chỗ khác).
...
Chỉ dùng khi ông chủ đang develop, **không** có file `ussdgw-prod-release`.
...
**Từ package `ussdgw-prod-release`** (classpath `lib/*`):
...
Package `ussdgw-prod-release` đã seed sẵn rule `*519#` trong `gateway/config-seed/`.
...
Từ `ussdgw-prod-release` (gateway + MAP client đã chạy):
...
| # | Lệnh | Làm gì |
|---|------|--------|
| 1 | `cd /opt/ussdgw-prod-release && source scripts/env.sh` | Biến `$HTTP_LOADTEST_DIR`, `$MAP_LOAD_DIR` |
...
| # | Lệnh | Làm gì |
|---|------|--------|
| 1 | `cd /opt/ussdgw-prod-release/tools/http-simulator/loadtest` | Tool push (có thể dùng chung venv với `09-start-http-as`) |
...
Simulator Swing GUI (XML thủ công): `tools/http-simulator/bin/run.sh` — vẫn có trong `ussdgw-prod-release/tools/http-simulator/`.
...
Package `ussdgw-prod-release` bật bridge sẵn trong [`gateway/config-seed/UssdManagement_ussdproperties.xml`](../../ussdgw-prod-release/gateway/config-seed/UssdManagement_ussdproperties.xml):
...
```bash
cd ussdgw-prod-release && ./scripts/05-start-grpc-as.sh   # hoặc AS với bridge-delay bên dưới
```
...
| # | Lệnh | Làm gì |
|---|------|--------|
| 1 | `cd /opt/ussdgw-prod-release/tools/grpc-as-tester && .venv/bin/python ussd_as_server.py --port 8443 --min-delay 1 --max-delay 100` | AS latency ngẫu nhiên — nuôi EWMA gate |
...
**Điều kiện:** Lab §4, `sessionbridgeenabled=true`, warmup **ON**, theo dõi `docker logs ussd-prod`, `map-*.csv`, CDR.
...
| `UnknownHostException: ussd-prod` | Host network thiếu /etc/hosts | Image mới + `USSDGW_HOSTNAME=ussd-prod` |
...
## Config đã khớp sẵn (package `ussdgw-prod-release`)
...
| File | Nội dung |
|------|----------|
| [`ussdgw-prod-release/README.md`](../../ussdgw-prod-release/README.md) | Tóm tắt package |
