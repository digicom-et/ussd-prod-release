...
|--------|-------------|------------|
| **[A] Package `ussdgw-prod-release`** | Deploy to production server, extract and run | ⭐ Easy — **read this section first** |
| **[B] Dev machine** | Build from source `ussdgateway` + `jSS7` | ⭐⭐⭐ Harder — end of document |

> **Offline package:** Pre-bundled at `ussdgw-prod-release/` — see [`ussdgw-prod-release/README.md`](../../ussdgw-prod-release/README.md).
...
- Common environment variables: `source ./scripts/env.sh` (or `PKG_ROOT=/opt/ussdgw-prod-release`).
...
# [A] Run with package `ussdgw-prod-release` (recommended)
...
- `python3` (3.9–3.12)
- RAM ≥ 6 GB
- Package file extracted, e.g.: `/opt/ussdgw-prod-release/`
...
```bash
cd /opt/ussdgw-prod-release          # change path if you installed elsewhere
chmod +x scripts/*.sh
...
Assume `cd /opt/ussdgw-prod-release` and `source ./scripts/env.sh`.
...
**Must see** container `ussd-prod` status `running` (or `healthy` after ~3–5 minutes).
...
|---------|-----------|------|
| `init` | `ussd-prod-init` | Runs once: seed `/opt/ussdgw/data` |
...
| # | Command | Purpose |
|---|---------|---------|
| 1 | `cd /opt/ussdgw-prod-release/gateway` | Enter compose directory |
| 2 | `docker compose up -d` | Start `init` (seed data) + `ussdgw` (WildFly) |
| 3 | `docker compose ps` | Container `ussd-prod` = running/healthy |
| 4 | `curl -fs http://localhost:8080/jolokia/version` | HTTP health — wait 3–5 minutes first time |
| 5 | `docker logs -f ussd-prod` | Monitor SLEE deploy (optional) |
...
| # | Command | Purpose |
|---|---------|---------|
| 1 | `cd /opt/ussdgw-prod-release/tools/grpc-as-tester` | Python AS directory + `menu_config.json` |
...
Stop AS: `kill $(cat /opt/ussdgw-prod-release/.grpc-as.pid)` or `./scripts/05-stop-grpc-as.sh`.
...
| # | Command | Purpose |
|---|---------|---------|
| 1 | `cd /opt/ussdgw-prod-release/tools/jss7-map-load` | MAP load client (JAR in `lib/`) |
...
| # | Command | Purpose |
|---|---------|---------|
| 1 | `cd /opt/ussdgw-prod-release/tools/grpc-as-tester` | Same venv as `ussd_as_server.py` |
...
| 3 | `cd /opt/ussdgw-prod-release/tools/grpc-as-tester` | Tool `grpc_push_client.py` |
...
| # | Command | Purpose |
|---|---------|---------|
| 1 | `kill $(cat /opt/ussdgw-prod-release/.grpc-as.pid 2>/dev/null) 2>/dev/null; rm -f .grpc-as.pid` | Stop gRPC AS |
| 2 | `kill $(cat /opt/ussdgw-prod-release/.http-as.pid 2>/dev/null) 2>/dev/null; rm -f .http-as.pid` | Stop HTTP Pull AS (if any) |
| 3 | `cd /opt/ussdgw-prod-release/gateway && docker compose down` | Stop gateway + init |
...
Table below maps **1:1** each script → manual command. `PKG=/opt/ussdgw-prod-release` (change if extracted elsewhere).
...
Use only when developing, **no** `ussdgw-prod-release` file.
...
**From package `ussdgw-prod-release`** (classpath `lib/*`):
...
Package `ussdgw-prod-release` pre-seeds rule `*519#` in `gateway/config-seed/`.
...
From `ussdgw-prod-release` (gateway + MAP client already running):
...
| # | Command | Purpose |
|---|---------|---------|
| 1 | `cd /opt/ussdgw-prod-release && source scripts/env.sh` | Variables `$HTTP_LOADTEST_DIR`, `$MAP_LOAD_DIR` |
...
| # | Command | Purpose |
|---|---------|---------|
| 1 | `cd /opt/ussdgw-prod-release/tools/http-simulator/loadtest` | Push tool (can share venv with `09-start-http-as`) |
...
Simulator Swing GUI (manual XML): `tools/http-simulator/bin/run.sh` — also in `ussdgw-prod-release/tools/http-simulator/`.
...
Package `ussdgw-prod-release` enables bridge in [`gateway/config-seed/UssdManagement_ussdproperties.xml`](../../ussdgw-prod-release/gateway/config-seed/UssdManagement_ussdproperties.xml):
...
```bash
cd ussdgw-prod-release && ./scripts/05-start-grpc-as.sh   # or AS with bridge-delay below
```
...
| # | Command | Purpose |
|---|---------|---------|
| 1 | `cd /opt/ussdgw-prod-release/tools/grpc-as-tester && .venv/bin/python ussd_as_server.py --port 8443 --min-delay 1 --max-delay 100` | Random AS latency — feeds EWMA gate |
...
**Prerequisites:** Lab §4, `sessionbridgeenabled=true`, warmup **ON**, monitor `docker logs ussd-prod`, `map-*.csv`, CDR.
...
| `UnknownHostException: ussd-prod` | Host network missing /etc/hosts | New image + `USSDGW_HOSTNAME=ussd-prod` |
...
## Pre-matched config (package `ussdgw-prod-release`)
...
| File | Content |
|------|---------|
| [`ussdgw-prod-release/README.md`](../../ussdgw-prod-release/README.md) | Package summary |
