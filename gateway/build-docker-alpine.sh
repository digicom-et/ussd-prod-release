#!/usr/bin/env bash
# =============================================================================
#  Build the USSD Gateway Alpine Docker image.
#
#  Produces:  restcomm-ussd-alpine:<USSD_VERSION>
#             restcomm-ussd-alpine:latest
#
#  Requirements:
...
#    USSD_VERSION=7.3.0-SNAPSHOT ./build-docker-alpine.sh
#    SAVE_TAR=1 ./build-docker-alpine.sh
# =============================================================================
set -euo pipefail
...
# ---- Pre-flight checks -------------------------------------------------------
say "USSD Gateway — Alpine image build"
...
for f in Dockerfile.alpine standalone.conf docker-entrypoint.sh \
...
# ---- Docker build ------------------------------------------------------------
say "Building image restcomm-ussd-alpine:${IMAGE_TAG} (this takes a few minutes)..."
docker build \
    --progress=plain \
    --build-arg "USSD_VERSION=${VERSION}" \
    -f Dockerfile.alpine \
    -t "restcomm-ussd-alpine:${IMAGE_TAG}" \
    -t "restcomm-ussd-alpine:latest" \
    .

# ---- Image size + savings report --------------------------------------------
say "Image sizes"
ALPINE_SIZE="$(docker images "restcomm-ussd-alpine:${IMAGE_TAG}" --format '{{.Size}}')"
printf "    %salpine%s : %s\n" "${C_DIM}" "${C_RST}" "${ALPINE_SIZE}"
...
if docker image inspect "${HEAVY_TAG}" >/dev/null 2>&1; then
    ...
    if [ "${H}" -gt 0 ]; then
        ...
        if [ "${SAVED}" -ge 0 ]; then
            ...
            warn "alpine image is LARGER than ${HEAVY_TAG} — investigate"
        fi
    fi
else
...
if [ "${SAVE_TAR:-0}" = "1" ]; then
    ...
    mkdir -p "${OUT_DIR}"
    OUT_TAR="${OUT_DIR}/restcomm-ussd-alpine-${IMAGE_TAG}.tar"
    say "Saving image to ${OUT_TAR}"
    docker save -o "${OUT_TAR}" "restcomm-ussd-alpine:${IMAGE_TAG}"
...
echo
echo "Next steps:"
echo "  docker run --rm -it restcomm-ussd-alpine:${IMAGE_TAG} /usr/lib/jvm/default-jvm/bin/java -version"
echo "  cd ../scripts && USSDGW_IMAGE_VARIANT=alpine ./build-package.sh"