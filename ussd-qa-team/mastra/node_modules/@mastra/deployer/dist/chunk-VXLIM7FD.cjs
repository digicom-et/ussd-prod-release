...
var Deps = class extends base.MastraBase {
  ...
  async writePnpmConfig(dir, options) {
    const workspaceYamlPath = path__default.default.join(dir, "pnpm-workspace.yaml");
    const lines = [
      "packages:",
      "  - '.'",
      "allowBuilds:",
    ...
    lines.push("");
    await fsPromises__default.default.writeFile(workspaceYamlPath, lines.join("\n"), "utf-8");
  }
...
${Object.entries(config.supportedArchitectures).map(([key, value]) => `  ${key}: ${JSON.stringify(value)}`).join("\n")}`
  ...
  async getPackageVersion() {
    const __filename = url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('chunk-VXLIM7FD.cjs', document.baseURI).href)));
...
//# sourceMappingURL=chunk-VXLIM7FD.cjs.map
//# sourceMappingURL=chunk-VXLIM7FD.cjs.map