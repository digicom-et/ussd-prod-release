...
var babel2__namespace = /*#__PURE__*/_interopNamespace(babel2);
...
async function captureDependenciesToOptimize(output, workspaceMap, projectRoot, initialDepsToOptimize, {
  ...
  async function checkTransitiveDependencies(internalMap, maxDepth = 10, currentDepth = 0) {
    ...
    for (const [dep, meta] of depsSnapshot) {
      ...
      try {
        const importerPath = output.facadeModuleId ? url.pathToFileURL(output.facadeModuleId).href : url.pathToFileURL(projectRoot).href;
        const resolvedPath = localPkg.resolveModule(dep, {
          paths: [importerPath]
        });
        if (!resolvedPath) {
...
function checkConfigExport(result) {
  const t2 = babel2__namespace.default.types;
    ...
    visitor: {
      ExportNamedDeclaration(path2) {
        ...
        if (t2.isVariableDeclaration(decl)) {
          ...
          if (t2.isIdentifier(varDecl?.id, { name: "mastra" }) && t2.isNewExpression(varDecl.init) && t2.isIdentifier(varDecl.init.callee, { name: "Mastra" })) {
            result.hasValidConfig = true;
          }
        }
        if (Array.isArray(path2.node.specifiers)) {
          for (const spec of path2.node.specifiers) {
            if (t2.isExportSpecifier(spec) && t2.isIdentifier(spec.exported, { name: "mastra" }) && t2.isIdentifier(spec.local) && mastraVars.has(spec.local.name)) {
              result.hasValidConfig = true;
            }
          }
        }
      ...
      VariableDeclaration(path2) {
        for (const decl of path2.node.declarations) {
          if (t2.isIdentifier(decl.id) && t2.isNewExpression(decl.init) && t2.isIdentifier(decl.init.callee, { name: "Mastra" })) {
...
function getStringValue(node) {
  if (!node) return null;
  if (babel2.types.isStringLiteral(node)) {
    return node.value;
  }
  if (babel2.types.isTemplateLiteral(node) && node.expressions.length === 0 && node.quasis.length === 1) {
    return node.quasis[0]?.value.cooked ?? null;
  }
  return null;
}
function extractTargetFromProperty(prop) {
  if (!babel2.types.isObjectProperty(prop)) return null;
  const key = prop.key;
  if (babel2.types.isIdentifier(key, { name: "target" }) || babel2.types.isStringLiteral(key) && key.value === "target") {
...
function isBindingFromPino(path2, identifierName) {
  ...
  if (bindingPath.isImportSpecifier()) {
    ...
    if (importDecl?.isImportDeclaration() && importDecl.node.source.value === "pino") {
      ...
      if (babel2.types.isIdentifier(imported) && imported.name === "default") return true;
      if (babel2.types.isStringLiteral(imported) && imported.value === "default") return true;
    }
    return false;
  }
  if (bindingPath.isVariableDeclarator()) {
    const init = bindingPath.node.init;
    if (babel2.types.isCallExpression(init) && babel2.types.isIdentifier(init.callee, { name: "require" }) && init.arguments.length === 1 && babel2.types.isStringLiteral(init.arguments[0]) && init.arguments[0].value === "pino") {
      return true;
    }
  }
  return false;
...
function isPinoTransportCall(path2) {
  ...
  if (!babel2.types.isMemberExpression(callee)) return false;
  if (!babel2.types.isIdentifier(callee.property, { name: "transport" })) return false;
  if (babel2.types.isIdentifier(callee.object)) {
    return isBindingFromPino(path2, callee.object.name);
  }
  if (babel2.types.isMemberExpression(callee.object) && babel2.types.isIdentifier(callee.object.object) && babel2.types.isIdentifier(callee.object.property, { name: "default" })) {
...
function extractTransportsFromArg(arg) {
  const targets = [];
  if (!babel2.types.isObjectExpression(arg)) return targets;
  for (const prop of arg.properties) {
    if (!babel2.types.isObjectProperty(prop)) continue;
    const key = prop.key;
    const keyName = babel2.types.isIdentifier(key) ? key.name : babel2.types.isStringLiteral(key) ? key.value : null;
    if (keyName === "target") {
      ...
      if (babel2.types.isArrayExpression(prop.value)) {
        for (const element of prop.value.elements) {
          if (babel2.types.isObjectExpression(element)) {
  ...
  return {
    ...
    visitor: {
      CallExpression(path2) {
        ...
        if (firstArg && !babel2.types.isSpreadElement(firstArg)) {
...
async function analyzeBundle(entries, mastraEntry, {
  ...
  await babel2__namespace.transformAsync(mastraConfig, {
