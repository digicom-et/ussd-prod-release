...
var STUB_PREFIX = "mastra-stub:";
var _stubbedExternals = null;
...
async function getParentPath(specifier, url) {
  ...
  const matchedPackage = Object.keys(importers).find((external) => chunk7PMC7SBC_cjs.isDependencyPartOfPackage(specifier, external));
  if (!matchedPackage) {
    return null;
  }
...
async function resolve(specifier, context, nextResolve) {
  ...
  if (stubbedExternals.length > 0) {
    const isStubbed = stubbedExternals.some((ext) => chunk7PMC7SBC_cjs.isDependencyPartOfPackage(specifier, ext));
    if (isStubbed) {
