...
var Bundler = class extends bundler.MastraBundler {
  ...
  createBundler(inputOptions, outputOptions) {
    return chunkFBL7GBNM_cjs.createBundler(inputOptions, outputOptions);
  }
  async getUserBundlerOptions(mastraEntryFile, outputDirectory) {
    ...
    try {
      const bundlerOptions = await chunkVZYB4EVX_cjs.getBundlerOptions(mastraEntryFile, outputDirectory);
  ...
  async analyze(entry, mastraFile, outputDirectory) {
    return await chunk47KJ3RKB_cjs.analyzeBundle(
      [].concat(entry),
      mastraFile,
      {
  ...
  async generateNpmLockfile(outputDir) {
    ...
    const nodeModulesTmp = path.join(outputDir, "node_modules.__tmp");
    let movedNodeModules = false;
    try {
      if (await fsExtra__default.default.pathExists(nodeModules)) {
        await fsExtra__default.default.move(nodeModules, nodeModulesTmp, { overwrite: true });
        movedNodeModules = true;
      }
      child_process.execSync("npm install --package-lock-only --force", {
        cwd: outputDir,
        stdio: "pipe",
        timeout: 6e4
      });
      ...
      if (movedNodeModules) {
        await promises.rm(nodeModules, { recursive: true, force: true });
        await fsExtra__default.default.move(nodeModulesTmp, nodeModules, { overwrite: true });
      }
    }
  }
  ...
  async getBundlerOptions(serverFile, mastraEntryFile, analyzedBundleInfo, toolsPaths, { enableSourcemap, enableEsmShim, externals }) {
    const { workspaceRoot } = await chunk47KJ3RKB_cjs.getWorkspaceInformation({ mastraEntryFile });
    ...
    for (const toolPath of toolsPaths) {
      ...
      for (const path$1 of expandedPaths) {
        if (await fsExtra__default.default.pathExists(path$1)) {
          ...
          this.logger.warn("Tool path does not exist, skipping", { path: path$1 });
        }
      }
    }
    return inputs;
  ...
  async _bundle(serverFile, mastraEntryFile, {
    ...
    try {
      ...
      analyzedBundleInfo = await chunk47KJ3RKB_cjs.analyzeBundle(
    ...
    for (const [dep, depInfo] of analyzedBundleInfo.externalDependencies) {
      if (analyzedBundleInfo.workspaceMap.has(dep) || !chunk7PMC7SBC_cjs.isBareModuleSpecifier(dep)) {
        continue;
      }
      let version = depInfo.version;
      let actualPackageName;
      try {
        let rootPath = await chunkI7TERDMR_cjs.getPackageRootPath(dep, projectRoot);
        if (!rootPath) {
          rootPath = await chunkI7TERDMR_cjs.getPackageRootPath(dep, undefined);
        }
        if (rootPath) {
...
export const tools = [${toolsExports.join(", ")}]`
      ...
      await this.generateNpmLockfile(path.join(outputDirectory, this.outputDir));
...
//# sourceMappingURL=chunk-CHZ76N6R.cjs.map
//# sourceMappingURL=chunk-CHZ76N6R.cjs.map