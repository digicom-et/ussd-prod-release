...
var _permissionPatternsPromise;
function loadPermissionPatterns() {
  if (!_permissionPatternsPromise) {
    _permissionPatternsPromise = import('@mastra/core/auth/ee').then((m) => m.PERMISSION_PATTERNS).catch(() => {
      console.error(
        "[@mastra/server] EE auth features require @mastra/core >= 1.6.0. Please upgrade: npm install @mastra/core@latest"
      );
      return void 0;
...
function getAuthProvider(mastra, isStudio) {
...
function isStudioRequest(request) {
  return chunkSXKQ7CSX_cjs.isStudioClientTypeHeader(request.headers.get(chunkSXKQ7CSX_cjs.MASTRA_CLIENT_TYPE_HEADER) ?? void 0);
}
...
function getRBACProvider(mastra, isStudio) {
  if (isStudio) {
    const studioConfig = mastra.getStudio?.();
    if (studioConfig?.rbac) {
      return studioConfig.rbac;
    }
  }
  const serverConfig = mastra.getServer?.();
  return serverConfig?.rbac;
}
function getFGAProvider(mastra, isStudio) {
  if (isStudio) {
    const studioConfig = mastra.getStudio?.();
    if (studioConfig?.fga) {
      return studioConfig.fga;
    }
  }
  const serverConfig = mastra.getServer?.();
  return serverConfig?.fga;
}
function implementsInterface(auth, method) {
  return auth !== null && typeof auth === "object" && typeof auth[method] === "function";
}
var GET_AUTH_CAPABILITIES_ROUTE = chunkDN2NUSL2_cjs.createPublicRoute({
  method: "GET",
  path: "/auth/capabilities",
  responseType: "json",
  responseSchema: chunkRR5IB5A2_cjs.capabilitiesResponseSchema,
  ...
  handler: async (ctx) => {
    try {
      const { mastra, request, routePrefix } = ctx;
      ...
      if (!("user" in capabilities) && chunk4PQR3D5Y_cjs.supportsSessionRefresh(auth)) {
        try {
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "Error getting auth capabilities");
    }
  }
});
...
var GET_CURRENT_USER_ROUTE = chunkDN2NUSL2_cjs.createPublicRoute({
  method: "GET",
  path: "/auth/me",
  responseType: "json",
  responseSchema: chunkRR5IB5A2_cjs.currentUserResponseSchema,
  ...
  handler: async (ctx) => {
    try {
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "Error getting current user");
    }
  }
});
var GET_SSO_LOGIN_ROUTE = chunkDN2NUSL2_cjs.createPublicRoute({
  method: "GET",
  path: "/auth/sso/login",
  responseType: "datastream-response",
  queryParamSchema: chunkRR5IB5A2_cjs.ssoLoginQuerySchema,
  ...
  handler: async (ctx) => {
    try {
      ...
      if (redirect_uri) {
        if (!redirect_uri.startsWith("http")) {
          ...
          try {
            ...
            const isLocalhost = redirectUrl.hostname === "localhost" || redirectUrl.hostname === "127.0.0.1" || redirectUrl.hostname === "[::1]";
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "Error initiating SSO login");
    }
  }
});
var GET_SSO_CALLBACK_ROUTE = chunkDN2NUSL2_cjs.createPublicRoute({
  method: "GET",
  path: "/auth/sso/callback",
  responseType: "datastream-response",
  queryParamSchema: chunkRR5IB5A2_cjs.ssoCallbackQuerySchema,
  ...
  handler: async (ctx) => {
    ...
    try {
      let auth = getAuthProvider(mastra, true);
...
var POST_LOGOUT_ROUTE = chunkDN2NUSL2_cjs.createPublicRoute({
  method: "POST",
  path: "/auth/logout",
  responseType: "datastream-response",
  summary: "Logout",
  ...
  handler: async (ctx) => {
    ...
    try {
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "Error logging out");
    }
  }
});
var POST_REFRESH_ROUTE = chunkDN2NUSL2_cjs.createPublicRoute({
  method: "POST",
  path: "/auth/refresh",
  responseType: "datastream-response",
  responseSchema: chunkRR5IB5A2_cjs.refreshResponseSchema,
  ...
  handler: async (ctx) => {
    ...
    try {
      ...
      if (!auth || !implementsInterface(auth, "refreshSession") || !implementsInterface(auth, "getSessionIdFromRequest")) {
        throw new chunk64ITUOXI_cjs.HTTPException(404, { message: "Session refresh not configured" });
      }
      const sessionId = auth.getSessionIdFromRequest(request);
      if (!sessionId) {
        throw new chunk64ITUOXI_cjs.HTTPException(401, { message: "No session" });
      }
      const newSession = await auth.refreshSession(sessionId);
      if (!newSession) {
        throw new chunk64ITUOXI_cjs.HTTPException(401, { message: "Session expired" });
      }
      const headers = new Headers({ "Content-Type": "application/json" });
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "Error refreshing session");
    }
  }
});
var POST_CREDENTIALS_SIGN_IN_ROUTE = chunkDN2NUSL2_cjs.createPublicRoute({
  method: "POST",
  path: "/auth/credentials/sign-in",
  responseType: "datastream-response",
  bodySchema: chunkRR5IB5A2_cjs.credentialsSignInBodySchema,
...
var POST_CREDENTIALS_SIGN_UP_ROUTE = chunkDN2NUSL2_cjs.createPublicRoute({
  method: "POST",
  path: "/auth/credentials/sign-up",
  responseType: "datastream-response",
  bodySchema: chunkRR5IB5A2_cjs.credentialsSignUpBodySchema,
...
var GET_ROLE_PERMISSIONS_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "GET",
  ...
  handler: async (ctx) => {
    try {
      ...
      const callerPermissions = requestContext?.get(chunkSXKQ7CSX_cjs.MASTRA_USER_PERMISSIONS_KEY) ?? [];
      ...
      if (error instanceof chunk64ITUOXI_cjs.HTTPException) throw error;
      return chunkZ7LCIYK7_cjs.handleError(error, "Error getting role permissions");
    }
  }
});
var GET_PERMISSION_PATTERNS_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "GET",
  path: "/auth/permission-patterns",
  requiresAuth: true,
  responseType: "json",
  responseSchema: chunkRR5IB5A2_cjs.permissionPatternsResponseSchema,
  summary: "List valid permission patterns",
  description: "Returns the authoritative list of valid permission-pattern strings. Used by Studio to validate the route\u2192permission literals it ships and to gate the sidebar.",
  tags: ["Auth"],
  handler: async () => {
...
exports.GET_PERMISSION_PATTERNS_ROUTE = GET_PERMISSION_PATTERNS_ROUTE;
...
//# sourceMappingURL=chunk-AJ4VAZFT.cjs.map
//# sourceMappingURL=chunk-AJ4VAZFT.cjs.map