...
chunk3YQ7NWF6_cjs.__toESM(chunk3YQ7NWF6_cjs.require_json_schema_traverse(), 1);
...
function schemaToJsonSchema(schema$1) {
  const standardSchema = schema.toStandardSchema(schema$1);
  return chunk3YQ7NWF6_cjs.standardSchemaToJSONSchema(standardSchema);
}
function convertToJsonSchema(spec) {
  const converted = {
  ...
  if (spec.requestParams?.path) {
    const pathSchema = schemaToJsonSchema(spec.requestParams.path);
  ...
  if (spec.requestParams?.query) {
    const querySchema = schemaToJsonSchema(spec.requestParams.query);
...
function generateOpenAPIDocument(routes, info) {
  ...
  routes.forEach((route) => {
    ...
    if (!paths[openapiPath]) {
      paths[openapiPath] = /* @__PURE__ */ Object.create(null);
    }
...
function isComplexType(schema) {
  let inner = schema;
  while (inner instanceof v4.ZodOptional || inner instanceof v4.ZodNullable) {
    inner = inner.unwrap();
  }
  return inner instanceof v4.ZodArray || inner instanceof v4.ZodRecord || inner instanceof v4.ZodObject;
}
...
function createPublicRoute(config) {
  return createRoute({
    ...config,
    requiresAuth: false
  });
...
//# sourceMappingURL=chunk-DN2NUSL2.cjs.map
//# sourceMappingURL=chunk-DN2NUSL2.cjs.map