...
var chunkTQIKCVFQ_cjs = require('./chunk-TQIKCVFQ.cjs');
var chunkZNP74O27_cjs = require('./chunk-ZNP74O27.cjs');
...
var deltaCursorSchema = v4.z.string().min(1).describe("Opaque cursor value for incremental polling");
var listModeSchema = v4.z.enum(["page", "delta"]).describe("List mode: 'page' | 'delta', defaults to 'page' when omitted.");
var deltaLimitSchema = v4.z.coerce.number().int().min(1).max(100).optional().describe("Maximum number of updates to return in one delta poll");
var defaultPaginationArgs = {
  page: 0,
  perPage: 10
};
var defaultDeltaLimit = 10;
function refineObservabilityListMode(value, ctx) {
  if (value.mode === "delta") {
    if (value.pagination !== void 0) {
...
var deltaInfoSchema = v4.z.object({
  limit: v4.z.number().describe("Maximum number of updates requested for this delta poll"),
  hasMore: v4.z.boolean().describe("True when more matching updates remain after this response")
...
var aggregationTypeSchema = v4.z.enum(["sum", "avg", "min", "max", "count", "count_distinct", "last"]).describe("Aggregation function");
...
var entityVersionIdField = v4.z.string().describe("Version ID of the entity that produced this signal (e.g., agent version, workflow version)");
var parentEntityVersionIdField = v4.z.string().describe("Version ID of the parent entity that produced this signal");
var rootEntityVersionIdField = v4.z.string().describe("Version ID of the root entity that produced this signal");
...
var logRecordSchema = v4.z.object({
  logId: v4.z.string().nullish().describe("Unique id for this log event"),
...
}).strict().superRefine(refineObservabilityListMode).transform(
  (value) => normalizeObservabilityListArgs(value, {
...
var scoreRecordSchema = v4.z.object({
  scoreId: v4.z.string().nullish().describe("Unique id for this score event"),
...
var feedbackRecordObjectSchema = v4.z.object({
  feedbackId: v4.z.string().nullish().describe("Unique id for this feedback event"),
...
v4.z.object({
  mode: listModeSchema.optional(),
  filters: v4.z.preprocess(normalizeLegacyFeedbackActor, feedbackFilterObjectSchema).optional(),
  pagination: paginationArgsSchema2.optional(),
...
var providerField = v4.z.string().describe("Model provider");
var modelField = v4.z.string().describe("Model");
var estimatedCostField = v4.z.number().describe("Estimated cost");
var costUnitField = v4.z.string().describe("Unit for the estimated cost (e.g., usd)");
var costMetadField = v4.z.record(v4.z.string(), v4.z.unknown()).nullish().describe("Structured costing metadata");
var metricRecordSchema = v4.z.object({
  metricId: v4.z.string().nullish().describe("Unique id for this metric event"),
...
var metricsOrderByFieldSchema = v4.z.enum(["timestamp"]).describe("Field to order by: 'timestamp'");
var metricsOrderBySchema = v4.z.object({
...
var listMetricsResponseSchema = v4.z.object({
  pagination: paginationInfoSchema.optional(),
  delta: deltaInfoSchema.optional(),
  deltaCursor: deltaCursorSchema.optional(),
...
var distinctColumnSchema = v4.z.enum(METRIC_DISTINCT_COLUMNS).optional().describe(
...
var getMetricBreakdownArgsSchema = v4.z.object({
  ...
  distinctColumn: distinctColumnSchema,
  filters: metricsFilterSchema.optional(),
  limit: v4.z.number().int().positive().max(1e3).optional().describe("Maximum number of groups to return (server-side TopK). Required for high-cardinality groupBy."),
...
var getMetricNamesArgsSchema = v4.z.object({
  ...
  limit: v4.z.coerce.number().int().min(1).optional().describe("Maximum number of names to return")
...
var getMetricLabelValuesArgsSchema = v4.z.object({
  ...
  limit: v4.z.coerce.number().int().min(1).optional().describe("Maximum number of values to return")
...
v4.z.object({}).describe("Arguments for getting entity types");
...
function createNewRoute(def, config) {
  ...
  return chunkDN2NUSL2_cjs.createRoute({
    ...def,
    ...schemas,
    responseType: "json",
    tags: ["Observability"],
    ...
    handler: (async (params) => {
      try {
        ...
        return chunkZ7LCIYK7_cjs.handleError(error, `Error calling: '${def.summary.toLocaleLowerCase()}'`);
      }
    })
  });
}
var LIST_LOGS = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.LIST_LOGS, {
  queryParamSchema: chunkTQIKCVFQ_cjs.createObservabilityListQuerySchema(logsFilterSchema, logsOrderBySchema),
  responseSchema: listLogsResponseSchema,
  handler: async ({ mastra, mode, after, limit, ...params }) => {
    const filters = chunkDN2NUSL2_cjs.pickParams(logsFilterSchema, params);
    const observabilityStore = await chunkTQIKCVFQ_cjs.getObservabilityStore(mastra);
    if (mode === "delta") {
      chunkTQIKCVFQ_cjs.assertObservabilityDeltaSupported(observabilityStore, chunkTQIKCVFQ_cjs.OBSERVABILITY_LIST_ENDPOINTS.logs);
    ...
    const pagination = chunkDN2NUSL2_cjs.pickParams(chunkZNP74O27_cjs.paginationArgsSchema, params);
    const orderBy = chunkDN2NUSL2_cjs.pickParams(logsOrderBySchema, params);
    return await observabilityStore.listLogs(
      mode === "page" ? { mode, filters, pagination, orderBy } : { filters, pagination, orderBy }
    );
  }
});
var LIST_SCORES = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.LIST_SCORES, {
  queryParamSchema: chunkTQIKCVFQ_cjs.createObservabilityListQuerySchema(scoresFilterSchema, scoresOrderBySchema),
  responseSchema: listScoresResponseSchema,
  handler: async ({ mastra, mode, after, limit, ...params }) => {
    const filters = chunkDN2NUSL2_cjs.pickParams(scoresFilterSchema, params);
    const observabilityStore = await chunkTQIKCVFQ_cjs.getObservabilityStore(mastra);
    if (mode === "delta") {
      chunkTQIKCVFQ_cjs.assertObservabilityDeltaSupported(observabilityStore, chunkTQIKCVFQ_cjs.OBSERVABILITY_LIST_ENDPOINTS.scores);
    ...
    const pagination = chunkDN2NUSL2_cjs.pickParams(chunkZNP74O27_cjs.paginationArgsSchema, params);
    const orderBy = chunkDN2NUSL2_cjs.pickParams(scoresOrderBySchema, params);
    return await observabilityStore.listScores(
      mode === "page" ? { mode, filters, pagination, orderBy } : { filters, pagination, orderBy }
    );
  }
});
var CREATE_SCORE = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.CREATE_SCORE, {
  ...
  handler: async ({ mastra, score }) => {
    ...
    await observabilityStore.createScore({
      score: { ...score, scoreId: score.scoreId ?? observability.generateSignalId(), timestamp: /* @__PURE__ */ new Date() }
    });
    return { success: true };
  }
});
var GET_SCORE = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.GET_SCORE, {
  pathParamSchema: v4.z.object({ scoreId: v4.z.string() }),
  responseSchema: v4.z.object({ score: scoreRecordSchema.nullable() }),
...
var GET_SCORE_AGGREGATE = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.GET_SCORE_AGGREGATE, {
  bodySchema: getScoreAggregateArgsSchema,
  responseSchema: getScoreAggregateResponseSchema,
  handler: async ({ mastra, ...params }) => {
    const args = chunkDN2NUSL2_cjs.pickParams(getScoreAggregateArgsSchema, params);
    const observabilityStore = await chunkTQIKCVFQ_cjs.getObservabilityStore(mastra);
    return await observabilityStore.getScoreAggregate(args);
  }
});
var GET_SCORE_BREAKDOWN = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.GET_SCORE_BREAKDOWN, {
  bodySchema: getScoreBreakdownArgsSchema,
  responseSchema: getScoreBreakdownResponseSchema,
  handler: async ({ mastra, ...params }) => {
    const args = chunkDN2NUSL2_cjs.pickParams(getScoreBreakdownArgsSchema, params);
    const observabilityStore = await chunkTQIKCVFQ_cjs.getObservabilityStore(mastra);
    return await observabilityStore.getScoreBreakdown(args);
  }
});
var GET_SCORE_TIME_SERIES = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.GET_SCORE_TIME_SERIES, {
  bodySchema: getScoreTimeSeriesArgsSchema,
  responseSchema: getScoreTimeSeriesResponseSchema,
  handler: async ({ mastra, ...params }) => {
    const args = chunkDN2NUSL2_cjs.pickParams(getScoreTimeSeriesArgsSchema, params);
    const observabilityStore = await chunkTQIKCVFQ_cjs.getObservabilityStore(mastra);
    return await observabilityStore.getScoreTimeSeries(args);
  }
});
var GET_SCORE_PERCENTILES = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.GET_SCORE_PERCENTILES, {
  bodySchema: getScorePercentilesArgsSchema,
  responseSchema: getScorePercentilesResponseSchema,
  handler: async ({ mastra, ...params }) => {
    const args = chunkDN2NUSL2_cjs.pickParams(getScorePercentilesArgsSchema, params);
    const observabilityStore = await chunkTQIKCVFQ_cjs.getObservabilityStore(mastra);
    return await observabilityStore.getScorePercentiles(args);
  }
});
var LIST_FEEDBACK = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.LIST_FEEDBACK, {
  queryParamSchema: chunkTQIKCVFQ_cjs.createObservabilityListQuerySchema(feedbackFilterSchema, feedbackOrderBySchema),
  responseSchema: listFeedbackResponseSchema,
  handler: async ({ mastra, mode, after, limit, ...params }) => {
    const filters = chunkDN2NUSL2_cjs.pickParams(feedbackFilterSchema, params);
    const observabilityStore = await chunkTQIKCVFQ_cjs.getObservabilityStore(mastra);
    if (mode === "delta") {
      chunkTQIKCVFQ_cjs.assertObservabilityDeltaSupported(observabilityStore, chunkTQIKCVFQ_cjs.OBSERVABILITY_LIST_ENDPOINTS.feedback);
    ...
    const pagination = chunkDN2NUSL2_cjs.pickParams(chunkZNP74O27_cjs.paginationArgsSchema, params);
    const orderBy = chunkDN2NUSL2_cjs.pickParams(feedbackOrderBySchema, params);
    return await observabilityStore.listFeedback(
      mode === "page" ? { mode, filters, pagination, orderBy } : { filters, pagination, orderBy }
    );
  }
});
var CREATE_FEEDBACK = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.CREATE_FEEDBACK, {
  ...
  handler: async ({ mastra, feedback }) => {
    ...
    await observabilityStore.createFeedback({
      feedback: { ...feedback, feedbackId: feedback.feedbackId ?? observability.generateSignalId(), timestamp: /* @__PURE__ */ new Date() }
    });
    return { success: true };
  }
});
var GET_FEEDBACK_AGGREGATE = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.GET_FEEDBACK_AGGREGATE, {
  bodySchema: getFeedbackAggregateArgsSchema,
  responseSchema: getFeedbackAggregateResponseSchema,
  handler: async ({ mastra, ...params }) => {
    const args = chunkDN2NUSL2_cjs.pickParams(getFeedbackAggregateArgsSchema, params);
    const observabilityStore = await chunkTQIKCVFQ_cjs.getObservabilityStore(mastra);
    return await observabilityStore.getFeedbackAggregate(args);
  }
});
var GET_FEEDBACK_BREAKDOWN = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.GET_FEEDBACK_BREAKDOWN, {
  bodySchema: getFeedbackBreakdownArgsSchema,
  responseSchema: getFeedbackBreakdownResponseSchema,
  handler: async ({ mastra, ...params }) => {
    const args = chunkDN2NUSL2_cjs.pickParams(getFeedbackBreakdownArgsSchema, params);
    const observabilityStore = await chunkTQIKCVFQ_cjs.getObservabilityStore(mastra);
    return await observabilityStore.getFeedbackBreakdown(args);
  }
});
var GET_FEEDBACK_TIME_SERIES = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.GET_FEEDBACK_TIME_SERIES, {
  bodySchema: getFeedbackTimeSeriesArgsSchema,
  responseSchema: getFeedbackTimeSeriesResponseSchema,
  handler: async ({ mastra, ...params }) => {
    const args = chunkDN2NUSL2_cjs.pickParams(getFeedbackTimeSeriesArgsSchema, params);
    const observabilityStore = await chunkTQIKCVFQ_cjs.getObservabilityStore(mastra);
    return await observabilityStore.getFeedbackTimeSeries(args);
  }
});
var GET_FEEDBACK_PERCENTILES = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.GET_FEEDBACK_PERCENTILES, {
  bodySchema: getFeedbackPercentilesArgsSchema,
  responseSchema: getFeedbackPercentilesResponseSchema,
  handler: async ({ mastra, ...params }) => {
    const args = chunkDN2NUSL2_cjs.pickParams(getFeedbackPercentilesArgsSchema, params);
    const observabilityStore = await chunkTQIKCVFQ_cjs.getObservabilityStore(mastra);
    return await observabilityStore.getFeedbackPercentiles(args);
  }
});
var LIST_METRICS = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.LIST_METRICS, {
  queryParamSchema: chunkTQIKCVFQ_cjs.createObservabilityListQuerySchema(metricsFilterSchema, metricsOrderBySchema),
  responseSchema: listMetricsResponseSchema,
  handler: async ({ mastra, mode, after, limit, ...params }) => {
    const filters = chunkDN2NUSL2_cjs.pickParams(metricsFilterSchema, params);
    const observabilityStore = await chunkTQIKCVFQ_cjs.getObservabilityStore(mastra);
    if (mode === "delta") {
      chunkTQIKCVFQ_cjs.assertObservabilityDeltaSupported(observabilityStore, chunkTQIKCVFQ_cjs.OBSERVABILITY_LIST_ENDPOINTS.metrics);
    ...
    const pagination = chunkDN2NUSL2_cjs.pickParams(chunkZNP74O27_cjs.paginationArgsSchema, params);
    const orderBy = chunkDN2NUSL2_cjs.pickParams(metricsOrderBySchema, params);
    return await observabilityStore.listMetrics(
      mode === "page" ? { mode, filters, pagination, orderBy } : { filters, pagination, orderBy }
    );
  }
});
var GET_METRIC_AGGREGATE = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.GET_METRIC_AGGREGATE, {
  bodySchema: getMetricAggregateArgsSchema,
  responseSchema: getMetricAggregateResponseSchema,
  handler: async ({ mastra, ...params }) => {
    const args = chunkDN2NUSL2_cjs.pickParams(getMetricAggregateArgsSchema, params);
    const observabilityStore = await chunkTQIKCVFQ_cjs.getObservabilityStore(mastra);
    return await observabilityStore.getMetricAggregate(args);
  }
});
var GET_METRIC_BREAKDOWN = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.GET_METRIC_BREAKDOWN, {
  bodySchema: getMetricBreakdownArgsSchema,
  responseSchema: getMetricBreakdownResponseSchema,
  handler: async ({ mastra, ...params }) => {
    const args = chunkDN2NUSL2_cjs.pickParams(getMetricBreakdownArgsSchema, params);
    const observabilityStore = await chunkTQIKCVFQ_cjs.getObservabilityStore(mastra);
    return await observabilityStore.getMetricBreakdown(args);
  }
});
var GET_METRIC_TIME_SERIES = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.GET_METRIC_TIME_SERIES, {
  bodySchema: getMetricTimeSeriesArgsSchema,
  responseSchema: getMetricTimeSeriesResponseSchema,
  handler: async ({ mastra, ...params }) => {
    const args = chunkDN2NUSL2_cjs.pickParams(getMetricTimeSeriesArgsSchema, params);
    const observabilityStore = await chunkTQIKCVFQ_cjs.getObservabilityStore(mastra);
    return await observabilityStore.getMetricTimeSeries(args);
  }
});
var GET_METRIC_PERCENTILES = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.GET_METRIC_PERCENTILES, {
  bodySchema: getMetricPercentilesArgsSchema,
  responseSchema: getMetricPercentilesResponseSchema,
  handler: async ({ mastra, ...params }) => {
    const args = chunkDN2NUSL2_cjs.pickParams(getMetricPercentilesArgsSchema, params);
    const observabilityStore = await chunkTQIKCVFQ_cjs.getObservabilityStore(mastra);
    return await observabilityStore.getMetricPercentiles(args);
  }
});
var GET_METRIC_NAMES = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.GET_METRIC_NAMES, {
  queryParamSchema: chunkDN2NUSL2_cjs.wrapSchemaForQueryParams(getMetricNamesArgsSchema.partial()),
  responseSchema: getMetricNamesResponseSchema,
  handler: async ({ mastra, ...params }) => {
    const args = getMetricNamesArgsSchema.parse(chunkDN2NUSL2_cjs.pickParams(getMetricNamesArgsSchema, params));
    const observabilityStore = await chunkTQIKCVFQ_cjs.getObservabilityStore(mastra);
    return await observabilityStore.getMetricNames(args);
  }
});
var GET_METRIC_LABEL_KEYS = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.GET_METRIC_LABEL_KEYS, {
  queryParamSchema: chunkDN2NUSL2_cjs.wrapSchemaForQueryParams(getMetricLabelKeysArgsSchema),
  responseSchema: getMetricLabelKeysResponseSchema,
  handler: async ({ mastra, ...params }) => {
    const args = getMetricLabelKeysArgsSchema.parse(chunkDN2NUSL2_cjs.pickParams(getMetricLabelKeysArgsSchema, params));
    const observabilityStore = await chunkTQIKCVFQ_cjs.getObservabilityStore(mastra);
    return await observabilityStore.getMetricLabelKeys(args);
  }
});
var GET_METRIC_LABEL_VALUES = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.GET_METRIC_LABEL_VALUES, {
  queryParamSchema: chunkDN2NUSL2_cjs.wrapSchemaForQueryParams(getMetricLabelValuesArgsSchema),
  responseSchema: getMetricLabelValuesResponseSchema,
  handler: async ({ mastra, ...params }) => {
    const args = getMetricLabelValuesArgsSchema.parse(chunkDN2NUSL2_cjs.pickParams(getMetricLabelValuesArgsSchema, params));
    const observabilityStore = await chunkTQIKCVFQ_cjs.getObservabilityStore(mastra);
    return await observabilityStore.getMetricLabelValues(args);
  }
});
var GET_ENTITY_TYPES = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.GET_ENTITY_TYPES, {
...
var GET_ENTITY_NAMES = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.GET_ENTITY_NAMES, {
  queryParamSchema: chunkDN2NUSL2_cjs.wrapSchemaForQueryParams(getEntityNamesArgsSchema.partial()),
  responseSchema: getEntityNamesResponseSchema,
  handler: async ({ mastra, ...params }) => {
    const args = getEntityNamesArgsSchema.parse(chunkDN2NUSL2_cjs.pickParams(getEntityNamesArgsSchema, params));
    const observabilityStore = await chunkTQIKCVFQ_cjs.getObservabilityStore(mastra);
    return await observabilityStore.getEntityNames(args);
  }
});
var GET_SERVICE_NAMES = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.GET_SERVICE_NAMES, {
...
var GET_ENVIRONMENTS = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.GET_ENVIRONMENTS, {
...
var GET_TAGS = createNewRoute(chunkTQIKCVFQ_cjs.NEW_ROUTE_DEFS.GET_TAGS, {
  queryParamSchema: chunkDN2NUSL2_cjs.wrapSchemaForQueryParams(getTagsArgsSchema.partial()),
  responseSchema: getTagsResponseSchema,
  handler: async ({ mastra, ...params }) => {
    const args = getTagsArgsSchema.parse(chunkDN2NUSL2_cjs.pickParams(getTagsArgsSchema, params));
    const observabilityStore = await chunkTQIKCVFQ_cjs.getObservabilityStore(mastra);
    try {
...
exports.CREATE_FEEDBACK = CREATE_FEEDBACK;
exports.CREATE_SCORE = CREATE_SCORE;
...
exports.GET_METRIC_LABEL_VALUES = GET_METRIC_LABEL_VALUES;
...
exports.LIST_METRICS = LIST_METRICS;
exports.LIST_SCORES = LIST_SCORES;
exports.NEW_ROUTES = NEW_ROUTES;
//# sourceMappingURL=chunk-5GQWTNAY.cjs.map
//# sourceMappingURL=chunk-5GQWTNAY.cjs.map