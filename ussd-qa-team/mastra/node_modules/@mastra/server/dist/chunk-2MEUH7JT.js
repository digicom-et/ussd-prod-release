...
function handleSignalRoutingError(error, defaultMessage) {
  if (error instanceof MastraError && error.category === ErrorCategory.USER && !error.status && !error.details?.status) {
...
var SEND_AGENT_SIGNAL_ROUTE = createRoute({
  method: "POST",
...
var SEND_AGENT_MESSAGE_ROUTE = createRoute({
  method: "POST",
...
var QUEUE_AGENT_MESSAGE_ROUTE = createRoute({
  method: "POST",
...
var STREAM_UNTIL_IDLE_GENERATE_ROUTE = createRoute({
  method: "POST",
...
var OBSERVE_AGENT_STREAM_ROUTE = createRoute({
  method: "POST",
  ...
  handler: async ({ mastra, agentId, runId, offset, abortSignal }) => {
    try {
      ...
      const agentPubsub = isDurableAgentLike(agent) ? agent.pubsub : void 0;
...
var LIST_SUSPENDED_RUNS_ROUTE = createRoute({
  method: "GET",
...
var RESUME_STREAM_ROUTE = createRoute({
  method: "POST",
  ...
  handler: async ({ mastra, agentId, abortSignal, requestContext: serverRequestContext, ...params }) => {
    try {
      ...
      await validateRunOwnership(workflowRun, getEffectiveResourceId(serverRequestContext, void 0));
      const { structuredOutput, untilIdle, ...restOptions } = rest;
      const options = {
        runId,
        toolCallId,
...
var RESUME_STREAM_UNTIL_IDLE_ROUTE = createRoute({
  method: "POST",
