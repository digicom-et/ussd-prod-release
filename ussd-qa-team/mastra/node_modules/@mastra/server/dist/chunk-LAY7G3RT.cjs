...
var chunkAR7VSXHH_cjs = require('./chunk-AR7VSXHH.cjs');
...
var builderRegistryEntrySchema = v4.z.object({
  id: v4.z.literal("skills-sh").describe("Stable registry identifier"),
  enabled: v4.z.boolean().describe("Whether this registry is enabled in the running deployment"),
  label: v4.z.string().describe("Human-readable registry name")
});
var builderRegistriesResponseSchema = v4.z.object({
  registries: v4.z.array(builderRegistryEntrySchema)
});
var builderRegistryPathParams = v4.z.object({
  registryId: v4.z.string().describe('Registry identifier (e.g. "skills-sh")')
});
var builderRegistrySearchQuerySchema = v4.z.object({
  q: v4.z.string().describe("Search query"),
  limit: v4.z.coerce.number().int().min(1).max(100).optional().default(10).describe("Maximum number of results (1-100)")
});
var builderRegistryPopularQuerySchema = v4.z.object({
  limit: v4.z.coerce.number().int().min(1).max(100).optional().default(10).describe("Maximum number of results (1-100)"),
  offset: v4.z.coerce.number().int().min(0).optional().default(0).describe("Offset for pagination (must be a multiple of `limit`)")
}).refine((args) => args.offset % args.limit === 0, {
  message: "offset must be a multiple of limit (the upstream registry pages by `limit`)",
  path: ["offset"]
});
var builderRegistryPreviewQuerySchema = v4.z.object({
  owner: v4.z.string().describe("GitHub repository owner"),
  repo: v4.z.string().describe("GitHub repository name"),
  path: v4.z.string().describe("Skill name within repo")
});
var builderRegistryInstallBodySchema = v4.z.object({
  owner: v4.z.string().describe("GitHub repository owner"),
  repo: v4.z.string().describe("GitHub repository name"),
  skillName: v4.z.string().describe("Skill name from the registry"),
  visibility: v4.z.enum(["private", "public"]).optional().describe("Visibility for the new stored skill")
});
var builderRegistryInstallResponseSchema = v4.z.object({
  storedSkillId: v4.z.string().describe("Id of the newly created stored skill"),
  name: v4.z.string().describe("Resolved skill name"),
  filesWritten: v4.z.number().describe("Number of files materialized into the skill version snapshot")
});
...
async function requireEnabledRegistry(mastra, registryId) {
  ...
  if (!match || !match.enabled) {
    throw new chunk64ITUOXI_cjs.HTTPException(404, { message: "Registry not found" });
  }
}
function buildFileTree(files) {
  const root = [];
  for (const file of files) {
    const safePath = chunkAR7VSXHH_cjs.assertSafeFilePath(file.path);
...
var LIST_BUILDER_REGISTRIES_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "GET",
...
var BUILDER_REGISTRY_SEARCH_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "GET",
...
var BUILDER_REGISTRY_POPULAR_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "GET",
...
var BUILDER_REGISTRY_PREVIEW_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "GET",
...
var BUILDER_REGISTRY_INSTALL_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  ...
  handler: async ({ mastra, requestContext, registryId, owner, repo, skillName, visibility: bodyVisibility }) => {
    try {
      ...
      const result = await chunkAR7VSXHH_cjs.fetchSkillFiles(owner, repo, skillName);
      if (!result || result.files.length === 0) {
        throw new chunk64ITUOXI_cjs.HTTPException(404, {
          message: `Could not find skill "${skillName}" in ${owner}/${repo}.`
        });
      }
      const safeSkillId = chunkAR7VSXHH_cjs.assertSafeSkillName(result.skillId);
      ...
      const id = chunkYWUVDFQZ_cjs.toSlug(resolvedName) || safeSkillId;
      if (!id) {
...
exports.BUILDER_REGISTRY_INSTALL_ROUTE = BUILDER_REGISTRY_INSTALL_ROUTE;
exports.BUILDER_REGISTRY_POPULAR_ROUTE = BUILDER_REGISTRY_POPULAR_ROUTE;
...
//# sourceMappingURL=chunk-LAY7G3RT.cjs.map
//# sourceMappingURL=chunk-LAY7G3RT.cjs.map