...
function stashVersionOverrides(ctx, versions) {
  if (!versions) return;
  const existingRaw = ctx.get(di.MASTRA_VERSIONS_KEY);
  const existing = existingRaw && typeof existingRaw === "object" && !Array.isArray(existingRaw) ? existingRaw : void 0;
  const merged = di.mergeVersionOverrides(existing, versions);
  if (merged) {
...
function ensureDefaultVersionStatus(ctx, versionOptions) {
  ...
  const updated = { ...existingRaw, defaultStatus: inferredStatus };
  ctx.set(di.MASTRA_VERSIONS_KEY, updated);
}
function getIsStudioFromContext(requestContext) {
  return requestContext.get(chunkSXKQ7CSX_cjs.MASTRA_IS_STUDIO_KEY) === true;
}
function mergeBodyRequestContext(serverRequestContext, bodyRequestContext) {
  ...
  for (const [key, value] of Object.entries(bodyRequestContext)) {
    if (chunkSXKQ7CSX_cjs.isReservedRequestContextKey(key)) continue;
...
async function getWorkspaceToolsFromAgent(agent, requestContext) {
  try {
    ...
    const isEnabled = async (toolName) => {
      return (await chunkSXKQ7CSX_cjs.resolveToolConfig(toolsConfig, toolName, configContext)).enabled;
    };
    if (workspace.filesystem) {
      if (await isEnabled(chunkSXKQ7CSX_cjs.WORKSPACE_TOOLS.FILESYSTEM.READ_FILE)) {
        tools.push(chunkSXKQ7CSX_cjs.WORKSPACE_TOOLS.FILESYSTEM.READ_FILE);
      }
      if (await isEnabled(chunkSXKQ7CSX_cjs.WORKSPACE_TOOLS.FILESYSTEM.LIST_FILES)) {
        tools.push(chunkSXKQ7CSX_cjs.WORKSPACE_TOOLS.FILESYSTEM.LIST_FILES);
      }
      if (await isEnabled(chunkSXKQ7CSX_cjs.WORKSPACE_TOOLS.FILESYSTEM.FILE_STAT)) {
        tools.push(chunkSXKQ7CSX_cjs.WORKSPACE_TOOLS.FILESYSTEM.FILE_STAT);
      }
      if (!isReadOnly) {
        if (await isEnabled(chunkSXKQ7CSX_cjs.WORKSPACE_TOOLS.FILESYSTEM.WRITE_FILE)) {
          tools.push(chunkSXKQ7CSX_cjs.WORKSPACE_TOOLS.FILESYSTEM.WRITE_FILE);
        }
        if (await isEnabled(chunkSXKQ7CSX_cjs.WORKSPACE_TOOLS.FILESYSTEM.EDIT_FILE)) {
          tools.push(chunkSXKQ7CSX_cjs.WORKSPACE_TOOLS.FILESYSTEM.EDIT_FILE);
        }
        if (await isEnabled(chunkSXKQ7CSX_cjs.WORKSPACE_TOOLS.FILESYSTEM.DELETE)) {
          tools.push(chunkSXKQ7CSX_cjs.WORKSPACE_TOOLS.FILESYSTEM.DELETE);
        }
        if (await isEnabled(chunkSXKQ7CSX_cjs.WORKSPACE_TOOLS.FILESYSTEM.MKDIR)) {
          tools.push(chunkSXKQ7CSX_cjs.WORKSPACE_TOOLS.FILESYSTEM.MKDIR);
        }
      }
      if (await isEnabled(chunkSXKQ7CSX_cjs.WORKSPACE_TOOLS.FILESYSTEM.GREP)) {
        tools.push(chunkSXKQ7CSX_cjs.WORKSPACE_TOOLS.FILESYSTEM.GREP);
      }
    }
    if (workspace.canBM25 || workspace.canVector) {
      if (await isEnabled(chunkSXKQ7CSX_cjs.WORKSPACE_TOOLS.SEARCH.SEARCH)) {
        tools.push(chunkSXKQ7CSX_cjs.WORKSPACE_TOOLS.SEARCH.SEARCH);
      }
      if (!isReadOnly && await isEnabled(chunkSXKQ7CSX_cjs.WORKSPACE_TOOLS.SEARCH.INDEX)) {
        tools.push(chunkSXKQ7CSX_cjs.WORKSPACE_TOOLS.SEARCH.INDEX);
      }
    }
    if (workspace.sandbox) {
      if (workspace.sandbox.executeCommand && await isEnabled(chunkSXKQ7CSX_cjs.WORKSPACE_TOOLS.SANDBOX.EXECUTE_COMMAND)) {
        tools.push(chunkSXKQ7CSX_cjs.WORKSPACE_TOOLS.SANDBOX.EXECUTE_COMMAND);
      }
    }
    return tools;
  } catch {
  ...
  if ("listAgents" in agent) {
    try {
      ...
      serializedAgentAgents = Object.entries(agents || {}).reduce(
        (acc, [key, agent2]) => {
          acc[key] = { id: agent2.id, name: agent2.name ?? key };
          return acc;
        },
        {}
      );
...
async function formatAgentList({
  ...
  if (agent.requestContextSchema) {
    try {
      serializedRequestContextSchema = chunkYWUVDFQZ_cjs.stringify(zodToJson.zodToJsonSchema(agent.requestContextSchema));
    } catch (error) {
  ...
  return {
    ...
    provider: typeof agent.model === "string" ? llm.parseModelString(agent.model).provider ?? llm$1?.getProvider() : llm$1?.getProvider(),
    modelId: typeof agent.model === "string" ? llm.parseModelString(agent.model).modelId : llm$1?.getModelId(),
...
async function getAgentFromSystem({
    ...
    if (Object.keys(agents || {}).length) {
      for (const [_, ag] of Object.entries(agents)) {
        try {
          ...
          if (subAgent instanceof agent.Agent) {
            agent$1 = subAgent;
            break;
          }
        } catch (error) {
  ...
  if (agent$1 && mastra.getEditor) {
    try {
      const editorAgent = mastra.getEditor()?.agent;
      if (editorAgent) {
        agent$1 = await editorAgent.applyStoredOverrides(
          agent$1,
  ...
  if (!agent$1) {
    ...
    try {
      agent$1 = await mastra.getEditor()?.agent.getById(agentId, versionOptions) ?? null;
    } catch (error) {
...
var LIST_AGENTS_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "GET",
  path: "/agents",
  responseType: "json",
  ...
  responseSchema: chunkZQ4GVMYG_cjs.listAgentsResponseSchema,
    ...
    try {
      ...
      if (fgaProvider && user) {
        ...
        const accessible = await fgaProvider.filterAccessible(
          ...
          agentList,
          "agent",
          chunk42M6Y2OD_cjs.MastraFGAPermissions.AGENTS_READ
        );
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "Error getting agents");
    }
  }
});
var GET_AGENT_BY_ID_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "GET",
  path: "/agents/:agentId",
  responseType: "json",
  pathParamSchema: chunkZQ4GVMYG_cjs.agentIdPathParams,
  queryParamSchema: chunkZQ4GVMYG_cjs.agentVersionQuerySchema,
  responseSchema: chunkZQ4GVMYG_cjs.serializedAgentSchema,
  ...
  handler: async ({ agentId, mastra, requestContext, status, versionId }) => {
    try {
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "Error getting agent");
    }
  }
});
var CLONE_AGENT_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  ...
  handler: async ({ agentId, mastra, newId, newName, metadata, authorId, requestContext }) => {
    try {
      const editor = mastra.getEditor();
      if (!editor) {
        return chunkZ7LCIYK7_cjs.handleError(new Error("Editor is not configured on the Mastra instance"), "Error cloning agent");
      }
      ...
      const cloneId = chunkYWUVDFQZ_cjs.toSlug(newId || `${agentId}-clone`);
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "Error cloning agent");
    }
  }
});
var GENERATE_AGENT_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  path: "/agents/:agentId/generate",
  responseType: "json",
  pathParamSchema: chunkZQ4GVMYG_cjs.agentIdPathParams,
  bodySchema: chunkZQ4GVMYG_cjs.agentExecutionBodySchema,
  responseSchema: chunkZQ4GVMYG_cjs.generateResponseSchema,
  ...
  handler: async ({ agentId, mastra, abortSignal, requestContext: serverRequestContext, ...params }) => {
    try {
      ...
      if (memoryOption) {
        ...
        const effectiveResourceId = chunk42M6Y2OD_cjs.getEffectiveResourceId(serverRequestContext, memoryOption.resource);
        const effectiveThreadId = chunk42M6Y2OD_cjs.getEffectiveThreadId(serverRequestContext, clientThreadId);
      ...
      const result = structuredOutput ? await agent.generate(messages, { ...options, structuredOutput }) : await agent.generate(messages, options);
      return result;
    } catch (error) {
      return chunkZ7LCIYK7_cjs.handleError(error, "Error generating from agent");
    }
  }
});
var GENERATE_LEGACY_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  path: "/agents/:agentId/generate-legacy",
  responseType: "json",
  pathParamSchema: chunkZQ4GVMYG_cjs.agentIdPathParams,
  bodySchema: chunkZQ4GVMYG_cjs.agentExecutionLegacyBodySchema,
  responseSchema: chunkZQ4GVMYG_cjs.generateResponseSchema,
  ...
  handler: async ({ mastra, agentId, abortSignal, requestContext, ...params }) => {
    try {
      ...
      const effectiveResourceId = chunk42M6Y2OD_cjs.getEffectiveResourceId(requestContext, clientResourceId);
...
var STREAM_GENERATE_LEGACY_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  ...
  handler: async ({ mastra, agentId, abortSignal, requestContext, ...params }) => {
    try {
      ...
      const streamResponse = rest.output ? streamResult.toTextStreamResponse() : streamResult.toDataStreamResponse({
        ...
        getErrorMessage: (error) => {
          if (error instanceof Error) {
            const safeMessage = error.message.replace(/([A-Za-z]:)?[\/][^\s,)]+/g, "<path>").replace(/\s+at\s+[^\n]*/g, "").replace(/Response body:.*/s, "").trim();
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "error streaming agent response");
    }
  }
});
async function buildProvidersList(mastra) {
  ...
  const defaultGatewayIds = new Set(llm.defaultGateways.map((gateway) => gateway.id));
  if (!blockExternalProviders) {
    for (const [id, provider] of Object.entries(llm.PROVIDER_REGISTRY)) {
...
var GET_PROVIDERS_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "GET",
  path: "/agents/providers",
  responseType: "json",
  responseSchema: chunkZQ4GVMYG_cjs.providersResponseSchema,
  ...
  handler: async ({ mastra }) => {
    try {
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "Error fetching providers");
    }
  }
});
var GENERATE_AGENT_VNEXT_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
...
var STREAM_GENERATE_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  ...
  handler: async ({ mastra, agentId, abortSignal, requestContext: serverRequestContext, ...params }) => {
    try {
      ...
      const streamResult = structuredOutput ? await agent.stream(messages, { ...options, structuredOutput }) : await agent.stream(messages, options);
...
var sendAgentSignalResponseSchema = v4.z.object({
  accepted: v4.z.literal(true),
  runId: v4.z.string(),
  signal: v4.z.any().optional()
});
function handleSignalRoutingError(error$1, defaultMessage) {
  if (error$1 instanceof error.MastraError && error$1.category === error.ErrorCategory.USER && !error$1.status && !error$1.details?.status) {
    throw new chunk64ITUOXI_cjs.HTTPException(400, { message: error$1.message, cause: error$1 });
  }
  return chunkZ7LCIYK7_cjs.handleError(error$1, defaultMessage);
}
var sendAgentMessageResponseSchema = sendAgentSignalResponseSchema;
var SEND_AGENT_SIGNAL_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  path: "/agents/:agentId/signals",
  responseType: "json",
  pathParamSchema: chunkZQ4GVMYG_cjs.agentIdPathParams,
  bodySchema: chunkZQ4GVMYG_cjs.sendAgentSignalBodySchema,
  ...
  handler: async ({
    ...
    try {
      ...
      ensureDefaultVersionStatus(serverRequestContext, versionOptions);
      const effectiveResourceId = chunk42M6Y2OD_cjs.getEffectiveResourceId(serverRequestContext, resourceId);
      const effectiveThreadId = chunk42M6Y2OD_cjs.getEffectiveThreadId(serverRequestContext, threadId);
      const ifIdleWithContext = {
        ifIdle: {
          ...ifIdle ?? {},
          streamOptions: { ...idleStreamOptions ?? {}, requestContext: serverRequestContext }
        }
      };
      if (effectiveThreadId && effectiveResourceId) {
        ...
        if (memory) {
          const thread = await memory.getThreadById({ threadId: effectiveThreadId });
          await chunk42M6Y2OD_cjs.validateThreadOwnership(thread, effectiveResourceId);
        }
      }
      if (typeof agent.sendSignal !== "function") {
        throw new chunk64ITUOXI_cjs.HTTPException(501, { message: "agent signals are not supported by this Mastra core version" });
      }
      const agentSignal = signal;
      if (runId) {
        const result2 = await agent.sendSignal(agentSignal, {
          runId,
          ...effectiveResourceId ? { resourceId: effectiveResourceId } : {},
          ...effectiveThreadId ? { threadId: effectiveThreadId } : {},
          ...ifActive ? { ifActive } : {}
        });
        const settled2 = await result2.accepted;
        const settledRunId2 = "runId" in settled2 ? settled2.runId : runId;
      ...
      if (!effectiveResourceId || !effectiveThreadId) {
        throw new chunk64ITUOXI_cjs.HTTPException(400, { message: "resourceId and threadId are required when runId is not provided" });
      }
      ...
      return result.signal === void 0 ? { accepted: true, runId: settledRunId } : { accepted: true, runId: settledRunId, signal: result.signal };
    } catch (error) {
...
async function handleAgentMessageRoute({
  ...
  if (typeof agent[methodName] !== "function") {
    throw new chunk64ITUOXI_cjs.HTTPException(501, { message: `agent ${methodName} is not supported by this Mastra core version` });
  }
  if (runId) {
    ...
    const settled2 = await result2.accepted;
    const settledRunId2 = settled2 && "runId" in settled2 ? settled2.runId : runId;
    return result2.signal === void 0 ? { accepted: true, runId: settledRunId2 } : { accepted: true, runId: settledRunId2, signal: result2.signal };
  }
  if (!effectiveResourceId || !effectiveThreadId) {
    throw new chunk64ITUOXI_cjs.HTTPException(400, { message: "resourceId and threadId are required when runId is not provided" });
...
var SEND_AGENT_MESSAGE_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  path: "/agents/:agentId/send-message",
  responseType: "json",
  pathParamSchema: chunkZQ4GVMYG_cjs.agentIdPathParams,
  bodySchema: chunkZQ4GVMYG_cjs.sendAgentMessageBodySchema,
...
var QUEUE_AGENT_MESSAGE_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  path: "/agents/:agentId/queue-message",
  responseType: "json",
  pathParamSchema: chunkZQ4GVMYG_cjs.agentIdPathParams,
  bodySchema: chunkZQ4GVMYG_cjs.queueAgentMessageBodySchema,
  responseSchema: sendAgentMessageResponseSchema,
...
var ABORT_AGENT_THREAD_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  path: "/agents/:agentId/threads/abort",
  responseType: "json",
  pathParamSchema: chunkZQ4GVMYG_cjs.agentIdPathParams,
  bodySchema: chunkZQ4GVMYG_cjs.abortAgentThreadBodySchema,
  responseSchema: chunkZQ4GVMYG_cjs.abortAgentThreadResponseSchema,
  ...
  handler: async ({ mastra, agentId, resourceId, threadId, requestContext: serverRequestContext }) => {
    try {
      ...
      const effectiveThreadId = chunk42M6Y2OD_cjs.getEffectiveThreadId(serverRequestContext, threadId);
      if (!effectiveThreadId) {
        throw new chunk64ITUOXI_cjs.HTTPException(400, { message: "threadId is required" });
      }
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "error aborting agent thread");
    }
  }
});
var SUBSCRIBE_AGENT_THREAD_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  ...
  pathParamSchema: chunkZQ4GVMYG_cjs.agentIdPathParams,
  bodySchema: chunkZQ4GVMYG_cjs.subscribeAgentThreadBodySchema,
  responseSchema: chunkZQ4GVMYG_cjs.streamResponseSchema,
  ...
  handler: async ({ mastra, agentId, resourceId, threadId, abortSignal, requestContext: serverRequestContext }) => {
    try {
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "error subscribing to agent thread");
    }
  }
});
var STREAM_UNTIL_IDLE_GENERATE_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  path: "/agents/:agentId/stream-until-idle",
  responseType: "stream",
  streamFormat: "sse",
  pathParamSchema: chunkZQ4GVMYG_cjs.agentIdPathParams,
  bodySchema: chunkZQ4GVMYG_cjs.streamUntilIdleBodySchema,
  responseSchema: chunkZQ4GVMYG_cjs.streamResponseSchema,
  ...
  handler: async ({ mastra, agentId, abortSignal, requestContext: serverRequestContext, ...params }) => {
    try {
      ...
      const streamResult = structuredOutput ? await agent.streamUntilIdle(messages, { ...options, structuredOutput }) : await agent.streamUntilIdle(messages, options);
      return streamResult.fullStream;
    } catch (error) {
      return chunkZ7LCIYK7_cjs.handleError(error, "error streaming agent response");
    }
...
var STREAM_GENERATE_VNEXT_DEPRECATED_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
...
var OBSERVE_AGENT_STREAM_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  path: "/agents/:agentId/observe",
  responseType: "stream",
  streamFormat: "sse",
  pathParamSchema: chunkZQ4GVMYG_cjs.agentIdPathParams,
  bodySchema: chunkZQ4GVMYG_cjs.observeAgentBodySchema,
  responseSchema: chunkZQ4GVMYG_cjs.observeAgentResponseSchema,
  ...
  handler: async ({ mastra, agentId, runId, offset, abortSignal }) => {
    try {
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "error observing agent stream");
    }
  }
});
var APPROVE_TOOL_CALL_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  path: "/agents/:agentId/approve-tool-call",
  responseType: "stream",
  streamFormat: "sse",
  pathParamSchema: chunkZQ4GVMYG_cjs.agentIdPathParams,
  bodySchema: chunkZQ4GVMYG_cjs.approveToolCallBodySchema,
  responseSchema: chunkZQ4GVMYG_cjs.toolCallResponseSchema,
  ...
  handler: async ({ mastra, agentId, abortSignal, requestContext, ...params }) => {
    try {
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "error approving tool call");
    }
  }
});
...
var SEND_TOOL_APPROVAL_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  path: "/agents/:agentId/send-tool-approval",
  responseType: "json",
  pathParamSchema: chunkZQ4GVMYG_cjs.agentIdPathParams,
  bodySchema: chunkZQ4GVMYG_cjs.sendToolApprovalBodySchema,
  responseSchema: chunkZQ4GVMYG_cjs.sendToolApprovalResponseSchema,
  ...
  handler: async ({ mastra, agentId, abortSignal, requestContext: serverRequestContext, ...params }) => {
    try {
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "error sending tool approval");
    }
  }
});
var LIST_SUSPENDED_RUNS_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "GET",
  path: "/agents/:agentId/suspended-runs",
  responseType: "json",
  pathParamSchema: chunkZQ4GVMYG_cjs.agentIdPathParams,
  queryParamSchema: chunkZQ4GVMYG_cjs.listSuspendedRunsQuerySchema,
  responseSchema: chunkZQ4GVMYG_cjs.listSuspendedRunsResponseSchema,
  ...
  handler: async ({ mastra, agentId, requestContext, ...query }) => {
    try {
      ...
      const effectiveResourceId = chunk42M6Y2OD_cjs.getEffectiveResourceId(requestContext, query.resourceId);
      const effectiveThreadId = chunk42M6Y2OD_cjs.getEffectiveThreadId(requestContext, query.threadId);
      if (effectiveThreadId) {
        const memory = await agent.getMemory({ requestContext });
        if (!memory) {
        ...
        const thread = await memory.getThreadById({ threadId: effectiveThreadId });
        if (!thread) {
          throw new chunk64ITUOXI_cjs.HTTPException(403, { message: "Access denied: thread not found" });
        }
        await chunk42M6Y2OD_cjs.enforceThreadAccess({
          mastra,
          requestContext,
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "error listing suspended runs");
    }
  }
});
var DECLINE_TOOL_CALL_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  path: "/agents/:agentId/decline-tool-call",
  responseType: "stream",
  streamFormat: "sse",
  pathParamSchema: chunkZQ4GVMYG_cjs.agentIdPathParams,
  bodySchema: chunkZQ4GVMYG_cjs.declineToolCallBodySchema,
  responseSchema: chunkZQ4GVMYG_cjs.toolCallResponseSchema,
  ...
  handler: async ({ mastra, agentId, abortSignal, requestContext, ...params }) => {
    try {
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "error declining tool call");
    }
  }
});
var RESUME_STREAM_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  path: "/agents/:agentId/resume-stream",
  responseType: "stream",
  streamFormat: "sse",
  pathParamSchema: chunkZQ4GVMYG_cjs.agentIdPathParams,
  bodySchema: chunkZQ4GVMYG_cjs.resumeStreamBodySchema,
  responseSchema: chunkZQ4GVMYG_cjs.streamResponseSchema,
  ...
  handler: async ({ mastra, agentId, abortSignal, requestContext: serverRequestContext, ...params }) => {
    try {
      ...
      await chunk42M6Y2OD_cjs.validateRunOwnership(workflowRun, chunk42M6Y2OD_cjs.getEffectiveResourceId(serverRequestContext, void 0));
      const { structuredOutput, untilIdle, ...restOptions } = rest;
      const options = {
        runId,
        toolCallId,
      ...
      const streamResult = structuredOutput ? await agent.resumeStream(resumeData, { ...options, structuredOutput }) : await agent.resumeStream(resumeData, options);
      return streamResult.fullStream;
    } catch (error) {
      return chunkZ7LCIYK7_cjs.handleError(error, "error resuming agent stream");
    }
  }
});
var RESUME_STREAM_UNTIL_IDLE_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  path: "/agents/:agentId/resume-stream-until-idle",
  responseType: "stream",
  streamFormat: "sse",
  pathParamSchema: chunkZQ4GVMYG_cjs.agentIdPathParams,
  bodySchema: chunkZQ4GVMYG_cjs.resumeStreamUntilIdleBodySchema,
  responseSchema: chunkZQ4GVMYG_cjs.streamResponseSchema,
  ...
  handler: async ({ mastra, agentId, abortSignal, requestContext: serverRequestContext, ...params }) => {
    try {
      ...
      const streamResult = structuredOutput ? await agent.resumeStreamUntilIdle(resumeData, { ...options, structuredOutput }) : await agent.resumeStreamUntilIdle(resumeData, options);
      return streamResult.fullStream;
    } catch (error) {
      return chunkZ7LCIYK7_cjs.handleError(error, "error resuming agent stream");
    }
...
var APPROVE_TOOL_CALL_GENERATE_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
...
var DECLINE_TOOL_CALL_GENERATE_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
...
var STREAM_NETWORK_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  ...
  handler: async ({ mastra, messages, agentId, requestContext, ...params }) => {
    try {
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "error streaming agent loop response");
    }
  }
});
var APPROVE_NETWORK_TOOL_CALL_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  path: "/agents/:agentId/approve-network-tool-call",
  responseType: "stream",
  streamFormat: "sse",
  pathParamSchema: chunkZQ4GVMYG_cjs.agentIdPathParams,
  bodySchema: chunkZQ4GVMYG_cjs.approveNetworkToolCallBodySchema,
  responseSchema: chunkZQ4GVMYG_cjs.streamResponseSchema,
  ...
  handler: async ({ mastra, agentId, requestContext, ...params }) => {
    try {
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "error approving network tool call");
    }
  }
});
var DECLINE_NETWORK_TOOL_CALL_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  path: "/agents/:agentId/decline-network-tool-call",
  responseType: "stream",
  streamFormat: "sse",
  pathParamSchema: chunkZQ4GVMYG_cjs.agentIdPathParams,
  bodySchema: chunkZQ4GVMYG_cjs.declineNetworkToolCallBodySchema,
  responseSchema: chunkZQ4GVMYG_cjs.streamResponseSchema,
  ...
  handler: async ({ mastra, agentId, requestContext, ...params }) => {
    try {
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "error declining network tool call");
    }
  }
});
var UPDATE_AGENT_MODEL_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  path: "/agents/:agentId/model",
  responseType: "json",
  pathParamSchema: chunkZQ4GVMYG_cjs.agentIdPathParams,
  bodySchema: chunkZQ4GVMYG_cjs.updateAgentModelBodySchema,
  responseSchema: chunkZQ4GVMYG_cjs.modelManagementResponseSchema,
  ...
  handler: async ({ mastra, agentId, modelId, provider }) => {
    try {
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "error updating agent model");
    }
  }
});
var RESET_AGENT_MODEL_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  ...
  handler: async ({ mastra, agentId }) => {
    try {
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "error resetting agent model");
    }
  }
});
var REORDER_AGENT_MODEL_LIST_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  path: "/agents/:agentId/models/reorder",
  responseType: "json",
  pathParamSchema: chunkZQ4GVMYG_cjs.agentIdPathParams,
  bodySchema: chunkZQ4GVMYG_cjs.reorderAgentModelListBodySchema,
  responseSchema: chunkZQ4GVMYG_cjs.modelManagementResponseSchema,
  ...
  handler: async ({ mastra, agentId, reorderedModelIds }) => {
    try {
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "error reordering model list");
    }
  }
});
var UPDATE_AGENT_MODEL_IN_MODEL_LIST_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  path: "/agents/:agentId/models/:modelConfigId",
  responseType: "json",
  pathParamSchema: chunkZQ4GVMYG_cjs.modelConfigIdPathParams,
  bodySchema: chunkZQ4GVMYG_cjs.updateAgentModelInModelListBodySchema,
  responseSchema: chunkZQ4GVMYG_cjs.modelManagementResponseSchema,
  ...
  handler: async ({ mastra, agentId, modelConfigId, model: bodyModel, maxRetries, enabled }) => {
    try {
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "error updating model in model list");
    }
  }
});
...
var ENHANCE_INSTRUCTIONS_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
  path: "/agents/:agentId/instructions/enhance",
  responseType: "json",
  pathParamSchema: chunkZQ4GVMYG_cjs.agentIdPathParams,
  bodySchema: chunkZQ4GVMYG_cjs.enhanceInstructionsBodySchema,
  responseSchema: chunkZQ4GVMYG_cjs.enhanceInstructionsResponseSchema,
...
${comment ? `User feedback: ${comment}` : ""}`,
        {
          structuredOutput: {
            schema: chunkZQ4GVMYG_cjs.enhanceInstructionsResponseSchema
          }
        }
      );
      return await result.object;
    } catch (error) {
      return chunkZ7LCIYK7_cjs.handleError(error, "Error enhancing instructions");
    }
  }
});
var STREAM_VNEXT_DEPRECATED_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
...
var STREAM_UI_MESSAGE_VNEXT_DEPRECATED_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
...
var STREAM_UI_MESSAGE_DEPRECATED_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "POST",
...
var GET_AGENT_SKILL_ROUTE = chunkDN2NUSL2_cjs.createRoute({
  method: "GET",
  path: "/agents/:agentId/skills/:skillName",
  responseType: "json",
  pathParamSchema: chunkZQ4GVMYG_cjs.agentSkillPathParams,
  ...
  handler: async ({ mastra, agentId, skillName, path, requestContext }) => {
    try {
      ...
      if (!skill) {
        throw new chunk64ITUOXI_cjs.HTTPException(404, { message: `Skill "${identifier}" not found` });
      }
      return {
        name: skill.name,
        description: skill.description,
      ...
      return chunkZ7LCIYK7_cjs.handleError(error, "Error getting agent skill");
    }
  }
});

exports.ABORT_AGENT_THREAD_ROUTE = ABORT_AGENT_THREAD_ROUTE;
...
exports.LIST_SUSPENDED_RUNS_ROUTE = LIST_SUSPENDED_RUNS_ROUTE;
exports.OBSERVE_AGENT_STREAM_ROUTE = OBSERVE_AGENT_STREAM_ROUTE;
exports.QUEUE_AGENT_MESSAGE_ROUTE = QUEUE_AGENT_MESSAGE_ROUTE;
...
exports.RESUME_STREAM_UNTIL_IDLE_ROUTE = RESUME_STREAM_UNTIL_IDLE_ROUTE;
exports.SEND_AGENT_MESSAGE_ROUTE = SEND_AGENT_MESSAGE_ROUTE;
exports.SEND_AGENT_SIGNAL_ROUTE = SEND_AGENT_SIGNAL_ROUTE;
exports.SEND_TOOL_APPROVAL_ROUTE = SEND_TOOL_APPROVAL_ROUTE;
...
exports.STREAM_UNTIL_IDLE_GENERATE_ROUTE = STREAM_UNTIL_IDLE_GENERATE_ROUTE;
exports.STREAM_VNEXT_DEPRECATED_ROUTE = STREAM_VNEXT_DEPRECATED_ROUTE;
exports.SUBSCRIBE_AGENT_THREAD_ROUTE = SUBSCRIBE_AGENT_THREAD_ROUTE;
...
exports.buildProvidersList = buildProvidersList;
exports.extractVersionOptions = extractVersionOptions;
exports.getAgentFromSystem = getAgentFromSystem;
exports.getBrowserToolsFromAgent = getBrowserToolsFromAgent;
...
//# sourceMappingURL=chunk-SCY53JIH.cjs.map
//# sourceMappingURL=chunk-SCY53JIH.cjs.map