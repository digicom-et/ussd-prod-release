...
function createObservabilityListQuerySchema(filterSchema, orderBySchema) {
  const unwrapDefault = (schema) => {
    ...
    return zodSchema instanceof v4.z.ZodDefault ? zodSchema.unwrap() : zodSchema;
  };
  const paginationShape = chunkZNP74O27_cjs.paginationArgsSchema.shape;
  ...
  const fieldSchema = orderByShape.field ? unwrapDefault(orderByShape.field) : v4.z.never().optional();
  const directionSchema = orderByShape.direction ? unwrapDefault(orderByShape.direction) : v4.z.never().optional();
  return chunkDN2NUSL2_cjs.wrapSchemaForQueryParams(
    v4.z.object({
      ...filterSchema.shape,
      page: pageSchema,
      ...
      mode: chunkZNP74O27_cjs.listModeSchema.optional(),
      after: chunkZNP74O27_cjs.deltaCursorSchema.optional(),
      limit: chunkZNP74O27_cjs.deltaLimitSchema
    }).partial()
...
//# sourceMappingURL=chunk-TQIKCVFQ.cjs.map
//# sourceMappingURL=chunk-TQIKCVFQ.cjs.map