...
var chunkFKM644Z6_cjs = require('../../chunk-FKM644Z6.cjs');
...
function getStoredResourceRouteFGAConfig(path, permission) {
  ...
  return {
    ...
    resourceId: (params, { requestContext }) => {
      ...
      const scopedResourceId = requestContext?.get(chunkSXKQ7CSX_cjs.MASTRA_RESOURCE_ID_KEY);
...
function getBuiltInRouteFGAConfig(route) {
  ...
  const permission = chunkFKM644Z6_cjs.getEffectivePermission(route);
  if (!permission) {
    return null;
  }
...
function serializeStreamChunk(chunk) {
  try {
    return { ok: true, json: utils.safeStringify(chunk) };
  } catch (error) {
...
function getFGAProvider(mastra, requestContext) {
  if (requestContext) {
    const authMode = requestContext.get(chunkSXKQ7CSX_cjs.MASTRA_AUTH_MODE_KEY);
    if (authMode === "studio") {
...
function getRoutePermissions(route) {
  return [chunkFKM644Z6_cjs.getEffectivePermission(route), route.fga?.permission].flatMap((value) => Array.isArray(value) ? value : [value]).filter((permission) => Boolean(permission));
}
...
function parseComplexQueryParams(queryParamSchema, params) {
  if (!(queryParamSchema instanceof v4.z.ZodObject)) {
    return params;
  }
  ...
  for (const [key, fieldSchema] of Object.entries(shape)) {
    ...
    const isComplex = typeName === "ZodObject" || typeName === "ZodArray" || typeName === "ZodRecord" || typeName === "object" || typeName === "array" || typeName === "record";
    if (!isComplex) {
      continue;
    }
    try {
...
var MastraServer = class _MastraServer extends server.MastraServerBase {
  mastra;
  bodyLimitOptions;
  tools;
  prefix;
  ...
  constructor({
    ...
    this.tools = tools;
    this.prefix = chunkYWUVDFQZ_cjs.normalizeRoutePath(prefix);
  ...
  applyRequestMetadataToContext({
    ...
    if (chunkSXKQ7CSX_cjs.isStudioClientTypeHeader(getHeader(chunkSXKQ7CSX_cjs.MASTRA_CLIENT_TYPE_HEADER))) {
      requestContext.set(chunkSXKQ7CSX_cjs.MASTRA_IS_STUDIO_KEY, true);
    }
  }
  /**
  ...
  getEffectiveAuthConfig(getHeader) {
    const isStudioRequest = chunkSXKQ7CSX_cjs.isStudioClientTypeHeader(getHeader(chunkSXKQ7CSX_cjs.MASTRA_CLIENT_TYPE_HEADER));
  ...
  async checkRouteAuth(route, context) {
    ...
    context.requestContext.set(chunkSXKQ7CSX_cjs.MASTRA_AUTH_MODE_KEY, authMode);
    ...
    const result = await chunk4PQR3D5Y_cjs.coreAuthMiddleware({
      path: context.path,
      ...
      rawRequest: context.request ?? new Request(`http://localhost${context.path}`, { method: context.method, headers: fallbackHeaders }),
      token,
  ...
  checkRoutePermission(route, userPermissions, hasPermissionFn, requestContext) {
    ...
    const requiredPermission = chunkFKM644Z6_cjs.getEffectivePermission(route);
...
Ensure @mastra/core is updated to a version that includes EE support.`
  ...
  async validateFGAPolicyCoverage() {
    ...
    const routes = [...chunkFKM644Z6_cjs.SERVER_ROUTES, ...customRoutes];
   ...
   */
  async handleCustomRouteRequest(url, method, headers, body, requestContext, signal) {
    ...
    const init = { method, headers: fetchHeaders, signal };
    if (["POST", "PUT", "PATCH", "DELETE"].includes(method) && body !== void 0) {
  ...
  async writeCustomRouteResponse(response, nodeRes, signal) {
    ...
    if (response.body) {
      ...
      try {
        ...
        if (signal) {
          await promises.pipeline(responseStream, nodeRes, { signal });
        } else {
          await promises.pipeline(responseStream, nodeRes);
        }
      } catch (error) {
  ...
  buildOpenAPISpec(config, prefix) {
    const openApiSpec = chunkDN2NUSL2_cjs.generateOpenAPIDocument(chunkFKM644Z6_cjs.SERVER_ROUTES, config);
    if (prefix) {
    ...
    if (allCustomRoutes && allCustomRoutes.length > 0) {
      const customPaths = chunkDN2NUSL2_cjs.convertCustomRoutesToOpenAPIPaths(allCustomRoutes);
      if (prefix) {
  ...
  async registerRoutes() {
    for (const route of chunkFKM644Z6_cjs.SERVER_ROUTES) {
  ...
  resolveValidationError(route, error, context) {
    ...
    return {
      status: 400,
      body: chunkZ7LCIYK7_cjs.formatZodError(error, _MastraServer.CONTEXT_LABELS[context])
    };
  }
};
async function checkRouteFGA(mastra, route, requestContext, params) {
  ...
  const effectivePermission = route.path ? chunkFKM644Z6_cjs.getEffectivePermission(route) : null;
...
Object.defineProperty(exports, "SERVER_ROUTES", {
  enumerable: true,
  get: function () { return chunkFKM644Z6_cjs.SERVER_ROUTES; }
});
Object.defineProperty(exports, "deriveAction", {
  enumerable: true,
  get: function () { return chunkFKM644Z6_cjs.deriveAction; }
});
Object.defineProperty(exports, "derivePermission", {
  enumerable: true,
  get: function () { return chunkFKM644Z6_cjs.derivePermission; }
});
Object.defineProperty(exports, "extractResource", {
  enumerable: true,
  get: function () { return chunkFKM644Z6_cjs.extractResource; }
});
Object.defineProperty(exports, "getEffectivePermission", {
  enumerable: true,
  get: function () { return chunkFKM644Z6_cjs.getEffectivePermission; }
});
Object.defineProperty(exports, "WorkflowRegistry", {
  enumerable: true,
  get: function () { return chunkYWUVDFQZ_cjs.WorkflowRegistry; }
});
Object.defineProperty(exports, "normalizeRoutePath", {
  enumerable: true,
  get: function () { return chunkYWUVDFQZ_cjs.normalizeRoutePath; }
});
Object.defineProperty(exports, "MASTRA_AUTH_MODE_KEY", {
  enumerable: true,
  get: function () { return chunkSXKQ7CSX_cjs.MASTRA_AUTH_MODE_KEY; }
});
Object.defineProperty(exports, "MASTRA_CLIENT_TYPE_HEADER", {
  enumerable: true,
  get: function () { return chunkSXKQ7CSX_cjs.MASTRA_CLIENT_TYPE_HEADER; }
});
Object.defineProperty(exports, "MASTRA_IS_STUDIO_KEY", {
  enumerable: true,
  get: function () { return chunkSXKQ7CSX_cjs.MASTRA_IS_STUDIO_KEY; }
});
Object.defineProperty(exports, "MASTRA_STUDIO_CLIENT_TYPE", {
  enumerable: true,
  get: function () { return chunkSXKQ7CSX_cjs.MASTRA_STUDIO_CLIENT_TYPE; }
});
Object.defineProperty(exports, "isReservedRequestContextKey", {
  enumerable: true,
  get: function () { return chunkSXKQ7CSX_cjs.isReservedRequestContextKey; }
});
Object.defineProperty(exports, "isStudioClientTypeHeader", {
  enumerable: true,
  get: function () { return chunkSXKQ7CSX_cjs.isStudioClientTypeHeader; }
});
Object.defineProperty(exports, "isZodError", {
  enumerable: true,
  get: function () { return chunkZ7LCIYK7_cjs.isZodError; }
});
Object.defineProperty(exports, "createPublicRoute", {
  enumerable: true,
  get: function () { return chunkDN2NUSL2_cjs.createPublicRoute; }
});
Object.defineProperty(exports, "createRoute", {
  enumerable: true,
  get: function () { return chunkDN2NUSL2_cjs.createRoute; }
});
Object.defineProperty(exports, "generateOpenAPIDocument", {
  enumerable: true,
  get: function () { return chunkDN2NUSL2_cjs.generateOpenAPIDocument; }
});
Object.defineProperty(exports, "jsonQueryParam", {
  enumerable: true,
  get: function () { return chunkDN2NUSL2_cjs.jsonQueryParam; }
});
Object.defineProperty(exports, "pickParams", {
  enumerable: true,
  get: function () { return chunkDN2NUSL2_cjs.pickParams; }
});
Object.defineProperty(exports, "wrapSchemaForQueryParams", {
  enumerable: true,
  get: function () { return chunkDN2NUSL2_cjs.wrapSchemaForQueryParams; }
});
