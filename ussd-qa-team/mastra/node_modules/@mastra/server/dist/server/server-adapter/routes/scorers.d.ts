export declare const SCORES_ROUTES: readonly [import(".").ServerRoute<{}, Record<string, {
    scorer: {
        config: {
            id: string;
            description: string;
            name?: string | undefined;
            type?: unknown;
            judge?: unknown;
        };
    };
    agentIds: string[];
    agentNames: string[];
    workflowIds: string[];
    isRegistered: boolean;
    source: "code" | "stored" | "fs";
    sampling?: Record<string, never> | undefined;
}>, "json", import(".").RouteSchemas<undefined, undefined, undefined, import("zod").ZodRecord<import("zod").ZodString, import("zod").ZodObject<{
    scorer: import("zod").ZodObject<{
        config: import("zod").ZodObject<{
            id: import("zod").ZodString;
            name: import("zod").ZodOptional<import("zod").ZodString>;
            description: import("zod").ZodString;
            type: import("zod").ZodOptional<import("zod").ZodUnknown>;
            judge: import("zod").ZodOptional<import("zod").ZodUnknown>;
        }, import("zod/v4/core").$strip>;
    }, import("zod/v4/core").$strip>;
    sampling: import("zod").ZodOptional<import("zod").ZodObject<{}, import("zod/v4/core").$strip>>;
    agentIds: import("zod").ZodArray<import("zod").ZodString>;
    agentNames: import("zod").ZodArray<import("zod").ZodString>;
    workflowIds: import("zod").ZodArray<import("zod").ZodString>;
    isRegistered: import("zod").ZodBoolean;
    source: import("zod").ZodEnum<{
        code: "code";
        stored: "stored";
        fs: "fs";
    }>;
}, import("zod/v4/core").$strip>>>, "GET", "/scores/scorers">, import(".").ServerRoute<{
    scorerId: string;
}, {
    scorer: {
        config: {
            id: string;
            description: string;
            name?: string | undefined;
            type?: unknown;
            judge?: unknown;
        };
    };
    agentIds: string[];
    agentNames: string[];
    workflowIds: string[];
    isRegistered: boolean;
    source: "code" | "stored" | "fs";
    sampling?: Record<string, never> | undefined;
} | null, "json", import(".").RouteSchemas<import("zod").ZodObject<{
    scorerId: import("zod").ZodString;
}, import("zod/v4/core").$strip>, undefined, undefined, import("zod").ZodNullable<import("zod").ZodObject<{
    scorer: import("zod").ZodObject<{
        config: import("zod").ZodObject<{
            id: import("zod").ZodString;
            name: import("zod").ZodOptional<import("zod").ZodString>;
            description: import("zod").ZodString;
            type: import("zod").ZodOptional<import("zod").ZodUnknown>;
            judge: import("zod").ZodOptional<import("zod").ZodUnknown>;
        }, import("zod/v4/core").$strip>;
    }, import("zod/v4/core").$strip>;
    sampling: import("zod").ZodOptional<import("zod").ZodObject<{}, import("zod/v4/core").$strip>>;
    agentIds: import("zod").ZodArray<import("zod").ZodString>;
    agentNames: import("zod").ZodArray<import("zod").ZodString>;
    workflowIds: import("zod").ZodArray<import("zod").ZodString>;
    isRegistered: import("zod").ZodBoolean;
    source: import("zod").ZodEnum<{
        code: "code";
        stored: "stored";
        fs: "fs";
    }>;
}, import("zod/v4/core").$strip>>>, "GET", "/scores/scorers/:scorerId">, import(".").ServerRoute<import(".").InferParams<import("zod").ZodObject<{
    runId: import("zod").ZodString;
}, import("zod/v4/core").$strip>, import("zod").ZodObject<{
    page: import("zod").ZodDefault<import("zod").ZodOptional<import("zod").ZodCoercedNumber<unknown>>>;
    perPage: import("zod").ZodDefault<import("zod").ZodOptional<import("zod").ZodCoercedNumber<unknown>>>;
}, import("zod/v4/core").$strip>, undefined>, {
    pagination: {
        total: number;
        page: number;
        perPage: number | false;
        hasMore: boolean;
    };
    scores: unknown[];
}, "json", import(".").RouteSchemas<import("zod").ZodObject<{
    runId: import("zod").ZodString;
}, import("zod/v4/core").$strip>, import("zod").ZodObject<{
    page: import("zod").ZodDefault<import("zod").ZodOptional<import("zod").ZodCoercedNumber<unknown>>>;
    perPage: import("zod").ZodDefault<import("zod").ZodOptional<import("zod").ZodCoercedNumber<unknown>>>;
}, import("zod/v4/core").$strip>, undefined, import("zod").ZodObject<{
    pagination: import("zod").ZodObject<{
        total: import("zod").ZodNumber;
        page: import("zod").ZodNumber;
        perPage: import("zod").ZodUnion<readonly [import("zod").ZodNumber, import("zod").ZodLiteral<false>]>;
        hasMore: import("zod").ZodBoolean;
    }, import("zod/v4/core").$strip>;
    scores: import("zod").ZodArray<import("zod").ZodUnknown>;
}, import("zod/v4/core").$strip>>, "GET", "/scores/run/:runId">, import(".").ServerRoute<import(".").InferParams<import("zod").ZodObject<{
    scorerId: import("zod").ZodString;
}, import("zod/v4/core").$strip>, import("zod").ZodObject<{
    page: import("zod").ZodDefault<import("zod").ZodOptional<import("zod").ZodCoercedNumber<unknown>>>;
    perPage: import("zod").ZodDefault<import("zod").ZodOptional<import("zod").ZodCoercedNumber<unknown>>>;
    entityId: import("zod").ZodOptional<import("zod").ZodString>;
    entityType: import("zod").ZodOptional<import("zod").ZodString>;
}, import("zod/v4/core").$strip>, undefined>, {
    pagination: {
        total: number;
        page: number;
        perPage: number | false;
        hasMore: boolean;
    };
    scores: unknown[];
}, "json", import(".").RouteSchemas<import("zod").ZodObject<{
    scorerId: import("zod").ZodString;
}, import("zod/v4/core").$strip>, import("zod").ZodObject<{
    page: import("zod").ZodDefault<import("zod").ZodOptional<import("zod").ZodCoercedNumber<unknown>>>;
    perPage: import("zod").ZodDefault<import("zod").ZodOptional<import("zod").ZodCoercedNumber<unknown>>>;
    entityId: import("zod").ZodOptional<import("zod").ZodString>;
    entityType: import("zod").ZodOptional<import("zod").ZodString>;
}, import("zod/v4/core").$strip>, undefined, import("zod").ZodObject<{
    pagination: import("zod").ZodObject<{
        total: import("zod").ZodNumber;
        page: import("zod").ZodNumber;
        perPage: import("zod").ZodUnion<readonly [import("zod").ZodNumber, import("zod").ZodLiteral<false>]>;
        hasMore: import("zod").ZodBoolean;
    }, import("zod/v4/core").$strip>;
    scores: import("zod").ZodArray<import("zod").ZodUnknown>;
}, import("zod/v4/core").$strip>>, "GET", "/scores/scorer/:scorerId">, import(".").ServerRoute<import(".").InferParams<import("zod").ZodObject<{
    entityType: import("zod").ZodString;
    entityId: import("zod").ZodString;
}, import("zod/v4/core").$strip>, import("zod").ZodObject<{
    page: import("zod").ZodDefault<import("zod").ZodOptional<import("zod").ZodCoercedNumber<unknown>>>;
    perPage: import("zod").ZodDefault<import("zod").ZodOptional<import("zod").ZodCoercedNumber<unknown>>>;
}, import("zod/v4/core").$strip>, undefined>, {
    pagination: {
        total: number;
        page: number;
        perPage: number | false;
        hasMore: boolean;
    };
    scores: unknown[];
}, "json", import(".").RouteSchemas<import("zod").ZodObject<{
    entityType: import("zod").ZodString;
    entityId: import("zod").ZodString;
}, import("zod/v4/core").$strip>, import("zod").ZodObject<{
    page: import("zod").ZodDefault<import("zod").ZodOptional<import("zod").ZodCoercedNumber<unknown>>>;
    perPage: import("zod").ZodDefault<import("zod").ZodOptional<import("zod").ZodCoercedNumber<unknown>>>;
}, import("zod/v4/core").$strip>, undefined, import("zod").ZodObject<{
    pagination: import("zod").ZodObject<{
        total: import("zod").ZodNumber;
        page: import("zod").ZodNumber;
        perPage: import("zod").ZodUnion<readonly [import("zod").ZodNumber, import("zod").ZodLiteral<false>]>;
        hasMore: import("zod").ZodBoolean;
    }, import("zod/v4/core").$strip>;
    scores: import("zod").ZodArray<import("zod").ZodUnknown>;
}, import("zod/v4/core").$strip>>, "GET", "/scores/entity/:entityType/:entityId">, import(".").ServerRoute<{
    score: unknown;
}, {
    score: unknown;
}, "json", import(".").RouteSchemas<undefined, undefined, import("zod").ZodObject<{
    score: import("zod").ZodUnknown;
}, import("zod/v4/core").$strip>, import("zod").ZodObject<{
    score: import("zod").ZodUnknown;
}, import("zod/v4/core").$strip>>, "POST", "/scores">];
//# sourceMappingURL=scorers.d.ts.map