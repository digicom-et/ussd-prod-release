import { getZodTypeName, getZodInnerType } from './chunk-TBHKQLPL.js';
import { SpanType, dbTimestamps, paginationInfoSchema, tagsField, metadataField, spanIdField, traceIdField, spanContextFields, dateRangeSchema, sortDirectionSchema, deltaLimitSchema, deltaCursorSchema, paginationArgsSchema, listModeSchema, refineObservabilityListMode, normalizeObservabilityListArgs, deltaInfoSchema } from './chunk-DEQCJWZZ.js';
import { z } from 'zod/v4';

var scoringSourceSchema = z.enum(["LIVE", "TEST"]);
var scoringEntityTypeSchema = z.enum([
  "AGENT",
  "WORKFLOW",
  "TRAJECTORY",
  "STEP",
...
var attributesField = z.record(z.string(), z.unknown()).describe("Span-type specific attributes (e.g., model, tokens, tools)");
...
var getSpansArgsSchema = z.object({
  traceId: traceIdField.min(1),
...
var getSpansResponseSchema = z.object({
  traceId: traceIdField,
...
var getBranchArgsSchema = z.object({
  traceId: traceIdField.min(1),
  spanId: spanIdField.min(1),
  depth: z.coerce.number().int().min(0).optional().describe("Maximum descendant levels below the anchor span (omit for full subtree)")
}).describe("Arguments for getting a span branch (subtree rooted at a span)");
var getBranchResponseSchema = z.object({
  traceId: traceIdField,
  spans: z.array(spanRecordSchema)
});
...
var lightSpanRecordSchema = z.object({
  // Required identifiers
  ...spanIds,
  name: spanNameField,
  spanType: spanTypeField,
...
var getStructureResponseSchema = z.object({
  traceId: traceIdField,
...
var listTracesLightResponseSchema = z.object({
  pagination: paginationInfoSchema,
  spans: z.array(lightSpanRecordSchema)
});
...
var branchesFilterSchema = z.object({
...
var branchesOrderByFieldSchema = z.enum(["startedAt", "endedAt"]).describe("Field to order by: 'startedAt' | 'endedAt'");
var branchesOrderBySchema = z.object({
  field: branchesOrderByFieldSchema.default("startedAt").describe("Field to order by"),
  direction: sortDirectionSchema.default("DESC").describe("Sort direction")
}).describe("Order by configuration");
var listBranchesArgsSchema = z.object({
  mode: listModeSchema.optional(),
  ...
  (value) => normalizeObservabilityListArgs(
    value,
    {
      orderBy: { field: "startedAt", direction: "DESC" }
    }
