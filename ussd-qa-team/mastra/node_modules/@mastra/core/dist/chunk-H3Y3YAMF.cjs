    ...
    outputStream = new ReadableStream({
      async start(controller) {
        try {
          ...
          chunk2PXTHATK_cjs.attachModelStreamTransport(outputStream, chunk2PXTHATK_cjs.readModelStreamTransport(stream));
...
var ObjectFormatHandler = class extends BaseFormatHandler {
  ...
  async processPartialChunk({
    ...
    const { value: currentObjectJson, state } = await chunkVRIGWPHF_cjs.parsePartialJson(processedAccumulatedText);
    if (this.validatePartialChunks && this.partialSchema) {
      const result = this.partialSchema?.safeParse(currentObjectJson);
      if (result.success && result.data && result.data !== void 0 && !chunkVRIGWPHF_cjs.isDeepEqualData(previousObject, result.data)) {
        return {
          shouldEmit: true,
    ...
    if (currentObjectJson !== void 0 && currentObjectJson !== null && typeof currentObjectJson === "object" && !chunkVRIGWPHF_cjs.isDeepEqualData(previousObject, currentObjectJson)) {
      return {
  ...
  async validateAndTransformFinal(finalRawValue) {
    ...
    const { value } = await chunkVRIGWPHF_cjs.parsePartialJson(rawValue);
...
var ArrayFormatHandler = class extends BaseFormatHandler {
  ...
  async processPartialChunk({
    ...
    const processedAccumulatedText = this.preprocessText(accumulatedText);
    const { value: currentObjectJson, state: parseState } = await chunkVRIGWPHF_cjs.parsePartialJson(processedAccumulatedText);
    if (currentObjectJson !== void 0 && !chunkVRIGWPHF_cjs.isDeepEqualData(previousObject, currentObjectJson)) {
      ...
      if (!chunkVRIGWPHF_cjs.isDeepEqualData(this.textPreviousFilteredArray, filteredElements)) {
...
var EnumFormatHandler = class extends BaseFormatHandler {
  ...
  async processPartialChunk({
    ...
    const processedAccumulatedText = this.preprocessText(accumulatedText);
    const { value: currentObjectJson } = await chunkVRIGWPHF_cjs.parsePartialJson(processedAccumulatedText);
    if (currentObjectJson !== void 0 && currentObjectJson !== null && typeof currentObjectJson === "object" && !Array.isArray(currentObjectJson) && "result" in currentObjectJson && typeof currentObjectJson.result === "string" && !chunkVRIGWPHF_cjs.isDeepEqualData(previousObject, currentObjectJson)) {
  ...
  async validateAndTransformFinal(rawFinalValue) {
    ...
    const { value } = await chunkVRIGWPHF_cjs.parsePartialJson(processedValue);
          ...
          async transform(chunk, controller) {
            if (chunk.type === "finish" && chunk.payload?.stepResult?.reason === "tool-calls") {
              ...
              const {
                ...
                chunk,
                processorStates,
                chunkSZ2THGT4_cjs.resolveObservabilityContext(options),
            ...
            case "finish":
              ...
              try {
                if (self.processorRunner && !self.#options.isLLMExecutionStep) {
                  ...
                  const outputText = lastResponseMessage ? chunkDC5C6QMM_cjs.coreContentToString(lastResponseMessage.content) : "";
...
// src/agent/utils.ts
var supportedLanguageModelSpecifications = ["v2", "v3", "v4"];
...
// src/processors/send-signal.ts
function createProcessorSendSignal(args) {
  return async (signalInput) => {
    const signal = chunkDC5C6QMM_cjs.createSignal(signalInput);
...
function areProcessorMessageArraysEqual(before, after) {
  ...
  return before.length === after.length && before.every((message, index) => chunkDC5C6QMM_cjs.messagesAreEqual(message, after[index]));
}
...
var ProcessorRunner = class _ProcessorRunner {
  ...
  async runComputeStateSignal({
    ...
    const trackingById = chunkCAGRA5LK_cjs.getStateSignalsMetadata(thread.metadata);
    const tracking = trackingById[stateId];
    const { activeStateSignals, contextWindow, lastSnapshot, deltasSinceSnapshot } = await chunkCAGRA5LK_cjs.resolveStateSignalHistory({
      messageList,
    ...
    const result = await computeStateSignal({
      ...
      sendStateSignal: async (stateSignal) => {
        const sendResult = await chunkCAGRA5LK_cjs.applyStateSignal({
          input: stateSignal,
          memory: resolvedMemory,
          thread,
          resourceId: resolvedResourceId,
    ...
    if (!result) return;
    await chunkCAGRA5LK_cjs.applyStateSignal({
      input: result,
      memory: resolvedMemory,
      thread,
      resourceId: resolvedResourceId,
    ...
    for (const [index, processorOrWorkflow] of this.outputProcessors.entries()) {
      ...
      try {
        ...
        const processResult = await processMethod({
          ...
          abort,
          agent: this.agent,
          ...chunkSZ2THGT4_cjs.createObservabilityContext({ currentSpan: processorSpan }),
          requestContext,
          retryCount,
          writer,
        ...
        if (processResult instanceof chunkDC5C6QMM_cjs.MessageList) {
        ...
        try {
          if (processor.processOutputStream && processedPart) {
            ...
            const result = await processor.processOutputStream({
              ...
              ...chunkSZ2THGT4_cjs.createObservabilityContext({ currentSpan: state.span }),
              requestContext,
              messageList,
              retryCount,
              writer
  ...
  async runInputProcessors(messageList, observabilityContext, requestContext, retryCount = 0) {
    for (const [index, processorOrWorkflow] of this.inputProcessors.entries()) {
      ...
      try {
        ...
        let mutations;
        if (result instanceof chunkDC5C6QMM_cjs.MessageList) {
  ...
  async runProcessInputStep(args) {
    ...
    const observabilityContext = chunkSZ2THGT4_cjs.resolveObservabilityContext(args);
    const stepInput = {
      ...
      try {
        ...
        const processMethodArgs = {
          ...
          sendStateSignal: async (stateSignal) => {
            ...
            const result2 = await chunkCAGRA5LK_cjs.applyStateSignal({
              input: stateSignal,
              memory: resolvedMemory,
              thread,
              resourceId: resolvedResourceId,
  ...
  async runProcessLLMRequest(args) {
    chunkSZ2THGT4_cjs.resolveObservabilityContext({ tracingContext: args.tracingContext });
    ...
    for (const processorOrWorkflow of this.inputProcessors) {
      ...
      try {
        ...
        const result = await processMethod({
          ...
          abortSignal: args.abortSignal,
          writer: args.writer,
          ...chunkSZ2THGT4_cjs.createObservabilityContext(args.tracingContext)
        });
  ...
  static async validateAndFormatProcessInputStepResult(result, {
    ...
    if (result instanceof chunkDC5C6QMM_cjs.MessageList) {
      ...
      if (result.model) {
        const resolvedModel = await chunk2PXTHATK_cjs.resolveModelConfig(result.model);
...
//# sourceMappingURL=chunk-H3Y3YAMF.cjs.map
//# sourceMappingURL=chunk-H3Y3YAMF.cjs.map