...
var chunkMB3TIFKT_cjs = require('./chunk-MB3TIFKT.cjs');
var chunkR4RZ3SPS_cjs = require('./chunk-R4RZ3SPS.cjs');
...
var ObservabilityStorage = class _ObservabilityStorage extends chunk2UAP4LDC_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "OBSERVABILITY"
  ...
  async getBranch(args) {
    const parsed = chunk3I6MKYQG_cjs.getBranchArgsSchema.parse(args);
    try {
      const skeleton = await this.getStructure({ traceId: parsed.traceId });
      if (!skeleton) return null;
      const branchSpanIds = chunk3I6MKYQG_cjs.extractBranchSpans(skeleton.spans, parsed.spanId, parsed.depth).map((s) => s.spanId);
    ...
    const spans = chunk3I6MKYQG_cjs.extractBranchSpans(trace.spans, parsed.spanId, parsed.depth);
...
function transformRow(row, tableName, options = {}) {
  ...
  const tableSchema = chunk3I6MKYQG_cjs.TABLE_SCHEMAS[tableName];
  const result = {};
...
function transformScoreRow(row, options = {}) {
  return transformRow(row, chunk3I6MKYQG_cjs.TABLE_SCORERS, options);
}
...
var ObservabilityInMemory = class extends ObservabilityStorage {
  ...
  deltaPollingFeatureEnabled() {
    return chunkR4RZ3SPS_cjs.coreFeatures.has(OBSERVABILITY_DELTA_POLLING_FEATURE);
  }
  ...
  maybeRegisterBranchCursor(span) {
    if (!chunk3I6MKYQG_cjs.BRANCH_SPAN_TYPE_SET.has(span.spanType)) {
      return;
    }
  ...
  getMatchingRootSpans(args) {
    const { filters, pagination, orderBy } = chunk3I6MKYQG_cjs.listTracesArgsSchema.parse(args);
  ...
  async listTraces(args) {
    const { mode, filters, after, limit } = chunk3I6MKYQG_cjs.listTracesArgsSchema.parse(args);
    if (mode === "delta") {
      ...
      return {
        spans: chunk3I6MKYQG_cjs.toTraceSpans(deltaResponse.rows),
    ...
    return {
      spans: chunk3I6MKYQG_cjs.toTraceSpans(paged),
  ...
  traceMatchesFilters(traceEntry, filters) {
    ...
    if (filters.startedAt) {
      if (filters.startedAt.start && (filters.startedAt.startExclusive ? rootSpan.startedAt <= filters.startedAt.start : rootSpan.startedAt < filters.startedAt.start)) {
        return false;
      }
      if (filters.startedAt.end && (filters.startedAt.endExclusive ? rootSpan.startedAt >= filters.startedAt.end : rootSpan.startedAt > filters.startedAt.end)) {
        return false;
      }
    }
    if (filters.endedAt) {
      ...
      if (filters.endedAt.start && (filters.endedAt.startExclusive ? rootSpan.endedAt <= filters.endedAt.start : rootSpan.endedAt < filters.endedAt.start)) {
        return false;
      }
      if (filters.endedAt.end && (filters.endedAt.endExclusive ? rootSpan.endedAt >= filters.endedAt.end : rootSpan.endedAt > filters.endedAt.end)) {
        return false;
      }
    }
  ...
  async listBranches(args) {
    const { mode, filters, pagination, orderBy, after, limit } = chunk3I6MKYQG_cjs.listBranchesArgsSchema.parse(args);
    if (mode === "delta") {
      this.assertDeltaPollingEnabled();
      ...
      return {
        branches: deltaResponse.rows.map(chunk3I6MKYQG_cjs.toTraceSpan),
        delta: deltaResponse.delta,
        deltaCursor: deltaResponse.deltaCursor
      };
    }
    const allowedSpanTypes = filters?.spanType ? chunk3I6MKYQG_cjs.BRANCH_SPAN_TYPE_SET.has(filters.spanType) ? /* @__PURE__ */ new Set([filters.spanType]) : /* @__PURE__ */ new Set() : chunk3I6MKYQG_cjs.BRANCH_SPAN_TYPE_SET;
    const matches = [];
    for (const [, traceEntry] of this.db.traces) {
    ...
    return {
      ...
      branches: paged.map(chunk3I6MKYQG_cjs.toTraceSpan),
  ...
  spanMatchesBranchFilters(span, filters) {
    ...
    if (filters.status !== void 0) {
      const spanStatus = chunk3I6MKYQG_cjs.toTraceSpan(span).status;
...
var BackgroundTasksStorage = class extends chunk2UAP4LDC_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
...
var ChannelsStorage = class extends chunk2UAP4LDC_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "CHANNELS"
...
var DatasetsStorage = class extends chunk2UAP4LDC_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "DATASETS"
...
var DatasetsInMemory = class extends DatasetsStorage {
  ...
  async listDatasets(args) {
    ...
    const { offset: start, perPage: perPageForResponse } = chunk2UAP4LDC_cjs.calculatePagination(page, perPageInput, perPage);
...
var ExperimentsStorage = class extends chunk2UAP4LDC_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "EXPERIMENTS"
...
var HarnessStorage = class extends chunk2UAP4LDC_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "HARNESS"
...
var MemoryStorage = class extends chunk2UAP4LDC_cjs.StorageDomain {
  /**
...
var InMemoryMemory = class extends MemoryStorage {
  ...
  async listMessages({
    ...
    const perPage = chunk2UAP4LDC_cjs.normalizePerPage(perPageInput, 40);
    if (page < 0) {
      throw new Error("page must be >= 0");
    }
  ...
  async listMessagesByResourceId({
    ...
    const list = new chunkDC5C6QMM_cjs.MessageList().add(
  ...
  async saveMessages(args) {
    ...
    const list = new chunkDC5C6QMM_cjs.MessageList().add(messages, "memory");
    return { messages: list.get.all.db() };
  }
...
var AGENT_THREAD_LEASE_TTL_MS = readPositiveIntEnv("MASTRA_AGENT_THREAD_LEASE_TTL_MS", 15e3);
var AGENT_THREAD_LEASE_RENEW_INTERVAL_MS = readPositiveIntEnv(
  "MASTRA_AGENT_THREAD_LEASE_RENEW_INTERVAL_MS",
  Math.floor(AGENT_THREAD_LEASE_TTL_MS / 3)
);
var defaultAgentThreadPubSub = new chunkMB3TIFKT_cjs.EventEmitterPubSub();
...
var AgentThreadStreamRuntime = class {
  ...
  #getLeaseProvider(pubsub) {
    ...
    if (typeof unwrap === "function") {
      ...
      return inner ?? chunkMB3TIFKT_cjs.NoopLeaseProvider;
    }
    return chunkMB3TIFKT_cjs.isLeaseProvider(resolved) ? resolved : chunkMB3TIFKT_cjs.NoopLeaseProvider;
  }
  #getSourceId() {
  ...
  #releaseThreadLease(pubsub, key, runId) {
    ...
    void this.#getLeaseProvider(resolved).releaseLease(key, runId).catch(() => {
    });
  }
  /**
  ...
  #startLeaseRenewal(pubsub, key, runId) {
    ...
    const timer = setInterval(() => {
      void leaseProvider.renewLease(key, runId, AGENT_THREAD_LEASE_TTL_MS).then((renewed) => {
        if (!renewed) {
  ...
  async #transferThreadLease(pubsub, key, fromRunId, toRunId) {
    ...
    const held = await leaseProvider.transferLease(key, fromRunId, toRunId, AGENT_THREAD_LEASE_TTL_MS).catch(() => false);
  ...
  async #acquireOrTransferThreadLease(pubsub, key, toRunId, fromRunId) {
    ...
    const leaseProvider = this.#getLeaseProvider(resolved);
    const result = await leaseProvider.acquireLease(key, toRunId, AGENT_THREAD_LEASE_TTL_MS).catch(() => ({ acquired: false, owner: void 0 }));
    if (result.acquired) {
      this.#startLeaseRenewal(resolved, key, toRunId);
  ...
  #nextStreamIdentity(state, runId) {
    ...
    return { streamId: crypto$1.randomUUID(), streamSeq };
  }
  ...
  resetForTests() {
    ...
    defaultAgentThreadPubSub = new chunkMB3TIFKT_cjs.EventEmitterPubSub();
  }
  #resetState(pubsub) {
  ...
  async #drainPendingSignals(state, pubsub, key, previousRun) {
    ...
    if (signal && queue) {
      ...
      const nextRunId = crypto$1.randomUUID();
  ...
  #startContinuation(state, pubsub, key, pending) {
    ...
    void pending.agent.stream(pending.messages, {
      ...
      this.#publish(pubsub, key, {
        type: "run-failed",
        runId: pending.runId,
        error: chunkXSOONORA_cjs.getErrorFromUnknown(err).message
      });
  ...
  async subscribeToThread(agent, options, pubsub) {
    ...
    const onEvent = (event) => {
      ...
      if (data.type === "signal-enqueued") {
        ...
        const queue = signalsByThread.get(key) ?? [];
        queue.push(chunkDC5C6QMM_cjs.createSignal(data.signal));
        signalsByThread.set(key, queue);
        return;
      }
      if (data.type === "run-failed") {
        const eventStreamId = data.streamId ?? data.runId;
        if (state.activeThreadRunIds.get(key) === data.runId && (!data.streamId || state.activeThreadStreamIds.get(key) === data.streamId)) {
          state.activeThreadRunIds.delete(key);
          state.activeThreadStreamIds.delete(key);
        }
        let errorRun;
      ...
      if (data.type === "run-completed" || data.type === "run-aborted" || data.type === "run-suspended") {
        ...
        } else if (state.activeThreadRunIds.get(key) === data.runId && (!data.streamId || state.activeThreadStreamIds.get(key) === data.streamId)) {
          state.activeThreadRunIds.delete(key);
          state.activeThreadStreamIds.delete(key);
        }
        if (data.type !== "run-suspended") {
          ...
          while (!done || pendingRuns.length > 0) {
            ...
            try {
              while (true) {
                ...
                const typedPart = part;
                const partWithRunId = typedPart && typeof typedPart === "object" && !("runId" in typedPart) ? { ...typedPart, runId: run.runId } : typedPart;
                yield partWithRunId;
                if (done) break;
                const finishReason = typedPart.finishReason ?? typedPart.payload?.finishReason;
                const terminalBoundary = typedPart.type === "error" || typedPart.type === "abort" || typedPart.type === "finish" && finishReason !== "tool-calls";
                if (terminalBoundary) {
                  readerReleased = true;
                  void (async () => {
                    try {
  ...
  sendMessage(agent, message, target, pubsub) {
    return this.sendSignal(agent, chunkDC5C6QMM_cjs.createMessageSignal(message, { acceptedAt: /* @__PURE__ */ new Date() }), target, pubsub);
  }
  queueMessage(agent, message, target, pubsub) {
    const state = this.#getState(pubsub);
    const signal = chunkDC5C6QMM_cjs.createMessageSignal(message, { acceptedAt: /* @__PURE__ */ new Date() });
    let key;
    ...
    if (activeRecord) {
      ...
      return {
        signal,
        accepted: Promise.resolve({ action: "deliver", runId: queuedRunId })
      };
    }
  ...
  async sendStateSignal(agent, stateInput, target, pubsub) {
    ...
    const memoryContext = chunkCAGRA5LK_cjs.parseMemoryRequestContext(requestContext);
    const memory = await agent.getMemory({ requestContext });
    if (!memory) {
    ...
    const applied = await chunkCAGRA5LK_cjs.applyStateSignal({
      input: stateInput,
      memory,
      thread,
      resourceId,
  ...
  sendSignal(agent, signalInput, target, pubsub) {
    const state = this.#getState(pubsub);
    let signal = chunkDC5C6QMM_cjs.createSignal({ ...signalInput, acceptedAt: /* @__PURE__ */ new Date() });
    let key;
    let runId = target.runId;
    ...
    const resourceId = target.resourceId ?? activeRecord?.resourceId;
    const threadId = target.threadId ?? activeRecord?.threadId;
    signal = chunkDC5C6QMM_cjs.resolveDeliveryAttributes(
      signal,
    ...
    if (state.activeThreadRunIds.has(key)) {
      ...
      if (this.#isSuspendedRun(state, blockingRunId) || blockingRecord?.output.status === "suspended" || blockingRecord?.lifecycle === "suspended") {
        return {
          signal,
          accepted: Promise.resolve({
            action: "blocked",
    ...
    const accepted = (async () => {
      const lease = await leaseProvider.acquireLease(reservedKey, reservedRunId, AGENT_THREAD_LEASE_TTL_MS).catch(() => ({ acquired: true, owner: reservedRunId }));
      if (!lease.acquired) {
        ...
        return { action: "deliver", runId: winnerRunId ?? reservedRunId };
      }
      ...
      try {
        ...
        return { action: "wake", runId: reservedRunId, output };
      } catch (error) {
        state.threadKeysByRunId.delete(reservedRunId);
        ...
        this.#publish(pubsub, reservedKey, {
          type: "run-failed",
          runId: reservedRunId,
          error: chunkXSOONORA_cjs.getErrorFromUnknown(error).message
        });
...
function createNotificationSignal(notification) {
  return chunkDC5C6QMM_cjs.createSignal({
    type: "notification",
...
var NotificationsStorage = class extends chunk2UAP4LDC_cjs.StorageDomain {
  constructor() {
...
var SchedulesStorage = class extends chunk2UAP4LDC_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "SCHEDULES"
...
var ScoresStorage = class extends chunk2UAP4LDC_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "SCORES"
...
var ScoresInMemory = class extends ScoresStorage {
  ...
  async listScoresByScorerId({
    ...
    const perPage = chunk2UAP4LDC_cjs.normalizePerPage(perPageInput, Number.MAX_SAFE_INTEGER);
    const { offset: start, perPage: perPageForResponse } = chunk2UAP4LDC_cjs.calculatePagination(page, perPageInput, perPage);
...
var WorkflowsStorage = class extends chunk2UAP4LDC_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "WORKFLOWS"
...
var WorkflowsInMemory = class extends WorkflowsStorage {
  ...
  async listWorkflowRuns({
    ...
    if (perPage !== void 0 && page !== void 0) {
      const normalizedPerPage = chunk2UAP4LDC_cjs.normalizePerPage(perPage, Number.MAX_SAFE_INTEGER);
...
// src/storage/mock.ts
var InMemoryStore = class extends chunk2UAP4LDC_cjs.MastraCompositeStore {
  stores;
  /**
  ...
  constructor({ id = "in-memory" } = {}) {
    ...
    this.#db = new chunkHLSFGO57_cjs.InMemoryDB();
    this.stores = {
      ...
      agents: new chunkHLSFGO57_cjs.InMemoryAgentsStorage({ db: this.#db }),
      ...
      promptBlocks: new chunkGGJVCX4I_cjs.InMemoryPromptBlocksStorage({ db: this.#db }),
      scorerDefinitions: new chunkEXFCDNO4_cjs.InMemoryScorerDefinitionsStorage({ db: this.#db }),
      mcpClients: new chunk4M6XWZDY_cjs.InMemoryMCPClientsStorage({ db: this.#db }),
      mcpServers: new chunkKWXEVPXK_cjs.InMemoryMCPServersStorage({ db: this.#db }),
      workspaces: new chunkLGSQSQFN_cjs.InMemoryWorkspacesStorage({ db: this.#db }),
      skills: new chunkZVZXKOMS_cjs.InMemorySkillsStorage({ db: this.#db }),
      ...
      threadState: new chunk2UAP4LDC_cjs.InMemoryThreadStateStorage()
    };
  }
  /**
...
var FilesystemStore = class extends chunk2UAP4LDC_cjs.MastraCompositeStore {
  #db;
  #dir;
  constructor(config = {}) {
    ...
    this.stores = {
      agents: new chunkHLSFGO57_cjs.FilesystemAgentsStorage({ db: this.#db }),
      promptBlocks: new chunkGGJVCX4I_cjs.FilesystemPromptBlocksStorage({ db: this.#db }),
      scorerDefinitions: new chunkEXFCDNO4_cjs.FilesystemScorerDefinitionsStorage({ db: this.#db }),
      mcpClients: new chunk4M6XWZDY_cjs.FilesystemMCPClientsStorage({ db: this.#db }),
      mcpServers: new chunkKWXEVPXK_cjs.FilesystemMCPServersStorage({ db: this.#db }),
      workspaces: new chunkLGSQSQFN_cjs.FilesystemWorkspacesStorage({ db: this.#db }),
      skills: new chunkZVZXKOMS_cjs.FilesystemSkillsStorage({ db: this.#db })
    };
  }
  /**
...
var StoreOperationsInMemory = class extends StoreOperations {
  ...
  async insert({ tableName, record }) {
    ...
    let key = record.id;
    if (tableName === chunk3I6MKYQG_cjs.TABLE_NOTIFICATIONS && record.threadId && record.id) {
      key = `${record.threadId}\0${record.id}`;
    } else if ([chunk3I6MKYQG_cjs.TABLE_WORKFLOW_SNAPSHOT].includes(tableName) && !record.id && record.run_id) {
...
//# sourceMappingURL=chunk-YLFDILPM.cjs.map
//# sourceMappingURL=chunk-YLFDILPM.cjs.map