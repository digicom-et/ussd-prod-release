...
function formatCommonBinaries(binaries) {
  if (!binaries?.length) return "";
  return binaries.map((binary) => `${binary.name}: ${binary.path ?? "not found"}`).join(", ");
}

// src/coding-agent/index.ts
var ECONNRESET_MAX_RETRIES = 2;
var ECONNRESET_RETRY_INITIAL_DELAY_MS = 1e3;
var ECONNRESET_RETRY_MAX_DELAY_MS = 3e4;
var ECONNRESET_MESSAGE_PATTERN = /econnreset|socket hang up/i;
...
function defaultErrorProcessors() {
  return [
    new StreamErrorRetryProcessor({
      matchers: [
        { match: isBadRequestError, maxRetries: 1, delayMs: 2e3 },
        {
          ...
          delayMs: ({ retryCount }) => Math.min(ECONNRESET_RETRY_INITIAL_DELAY_MS * Math.pow(2, retryCount), ECONNRESET_RETRY_MAX_DELAY_MS)
        }
      ]
    }),
...
function createCodingAgent(config) {
  ...
  const resolvedGoal = goal ? { ...goal, prompt: goal.prompt ?? DEFAULT_GOAL_JUDGE_PROMPT } : void 0;
  return new Agent({
    ...rest,
    workspace,
...
export { buildBasePrompt, createCodingAgent };
