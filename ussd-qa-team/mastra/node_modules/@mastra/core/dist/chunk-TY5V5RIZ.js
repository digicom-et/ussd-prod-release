...
${validation.errors.join("\n")}`);
   ...
   */
  #getParentPath(path3) {
    const lastSlash = Math.max(path3.lastIndexOf("/"), path3.lastIndexOf("\\"));
...
function isSkillFilePath(path3) {
  return path3 === "SKILL.md" || /[\\/]SKILL\.md$/.test(path3);
}
...
// src/skills/agent-skills-resolver.ts
var CompositeSkillSource = class {
  ...
  #route(path3) {
    return path3.startsWith("inline/") ? this.#inline : this.#local;
  }
  exists(path3) {
    return this.#route(path3).exists(path3);
  }
  stat(path3) {
    return this.#route(path3).stat(path3);
  }
  readFile(path3) {
    return this.#route(path3).readFile(path3);
  }
  readdir(path3) {
    return this.#route(path3).readdir(path3);
  }
  async realpath(path3) {
    const source = this.#route(path3);
    return source.realpath ? source.realpath(path3) : path3;
  }
};
...
export { DirectoryNotEmptyError, DirectoryNotFoundError, FileExistsError, FileNotFoundError, FileReadRequiredError, FilesystemError, FilesystemNotAvailableError, FilesystemNotReadyError, InlineSkillSource, IsDirectoryError, LocalSkillSource, NotDirectoryError, PermissionError, SandboxFeatureNotSupportedError, SandboxNotAvailableError, SearchNotAvailableError, StaleFileError, WorkspaceError, WorkspaceNotAvailableError, WorkspaceNotReadyError, WorkspaceReadOnlyError, WorkspaceSkillsImpl, createGlobMatcher, createSkill, expandTilde, extractGlobBase, fsExists, fsStat, isEexistError, isEnoentError, isGlobPattern, isInlineSkill, isTextFile, matchGlob, mergeWorkspaceSkills, resolveAgentSkills, resolvePathPattern, resolveToBasePath };
//# sourceMappingURL=chunk-TY5V5RIZ.js.map
//# sourceMappingURL=chunk-TY5V5RIZ.js.map