...
var chunkSKWYPONI_cjs = require('../chunk-SKWYPONI.cjs');
...
function extractTaskText(task) {
  const artifactText = (task.artifacts ?? []).flatMap(
    (artifact) => artifact.parts?.flatMap((part) => {
...
function messagesToPrompt(messages, options) {
  ...
  const converted = chunkDC5C6QMM_cjs.convertMessages(messages).to("AIV5.Model");
  const lines = [];
...
${options.instructions}`);
  ...
  for (const message of converted) {
    if (message.role === "system") {
      const content2 = Array.isArray(message.content) ? message.content.map((part) => {
...
function createResponseMessages(text, memoryInfo = {}) {
  ...
  return new chunkDC5C6QMM_cjs.MessageList(memoryInfo).add(
    {
      role: "assistant",
      content: text
    },
...
function unwrapA2AResult(result) {
  if (!result || typeof result !== "object") {
    throw chunkSKWYPONI_cjs.MastraA2AError.invalidAgentResponse("Remote A2A agent returned an invalid response.");
  }
...
async function requireResponseBody(response, operation) {
  if (!response.body) {
    throw chunkSKWYPONI_cjs.MastraA2AError.invalidAgentResponse(`Remote A2A agent returned an empty stream for ${operation}.`);
  }
...
var A2AAgent = class {
  ...
  async getMemory({ requestContext = new chunkPJIAL3WK_cjs.RequestContext() } = {}) {
    if (!this.#memory) {
      return void 0;
    }
  ...
  async resumeGenerate(resumeData, options) {
    ...
    if (!runId) {
      throw chunkSKWYPONI_cjs.MastraA2AError.invalidParams("A2AAgent.resumeGenerate requires a runId.");
    }
    const state = this.#runState.get(runId);
    if (!state) {
      throw chunkSKWYPONI_cjs.MastraA2AError.invalidParams(`No resumable A2A run state found for runId "${runId}".`);
    }
    const bootstrap = await this.#getBootstrap();
    const memoryInfo = resolveMemoryInfo(options);
    ...
    if (!state.taskId) {
      throw chunkSKWYPONI_cjs.MastraA2AError.invalidParams(`A2AAgent resume state for "${runId}" is missing a taskId.`);
    }
  ...
  async resumeStream(resumeData, options) {
    const runId = options?.runId;
    if (!runId) {
      throw chunkSKWYPONI_cjs.MastraA2AError.invalidParams("A2AAgent.resumeStream requires a runId.");
    }
    const state = this.#runState.get(runId);
    if (!state) {
      throw chunkSKWYPONI_cjs.MastraA2AError.invalidParams(`No resumable A2A run state found for runId "${runId}".`);
  ...
  async #getBootstrap({ forceRefresh = false } = {}) {
    ...
    if (this.#verifyAgentCard) {
      const context = { cardUrl, fetchedAt };
  ...
  async #getTask({
    ...
    if (!isTask(result)) {
      throw chunkSKWYPONI_cjs.MastraA2AError.invalidAgentResponse("Remote A2A agent returned a non-task response for tasks/get.");
    }
    return result;
  }
  ...
  async #consumeA2AStream({
    ...
    const messageList = new chunkDC5C6QMM_cjs.MessageList({ threadId, resourceId });
      ...
      while (next.eventBlock !== void 0) {
        ...
        if ("event" in parsed && parsed.event) {
          ...
          if (isTask(event)) {
            ...
            const text = event.artifact.parts?.flatMap(
              (part) => part.kind === "text" && "text" in part && typeof part.text === "string" ? [part.text] : []
            ).join("");
            if (text) {
      ...
      while (next.eventBlock !== void 0) {
        ...
        if ("event" in parsed && parsed.event) {
          ...
          if (isTask(event)) {
            ...
            const artifactText = event.artifact.parts?.flatMap(
              (part) => part.kind === "text" && "text" in part && typeof part.text === "string" ? [part.text] : []
            ).join("");
            if (artifactText) {
  ...
  async #request(url, { method = "POST", headers = {}, body, stream = false, credentials, signal } = {}) {
    ...
    while (attempts <= this.#retries) {
      try {
        ...
        if (!response.ok) {
          throw chunkSKWYPONI_cjs.MastraA2AError.invalidAgentResponse(`Remote A2A request failed with status ${response.status}.`, {
  ...
  #resolveRequestSignal(signal) {
    ...
    const signals = [signal, this.#abortSignal, timeoutSignal].filter(Boolean);
    if (signals.length === 0) {
      return void 0;
    }
...
Object.defineProperty(exports, "MastraA2AError", {
  enumerable: true,
  get: function () { return chunkSKWYPONI_cjs.MastraA2AError; }
});
