...
import type { TextStreamPart } from '../_types/@internal_ai-sdk-v4/dist/index.d.ts';
...
import type { AnySpan, ObservabilityContext, TracingOptions, TracingPolicy, TracingProperties } from '../observability/index.js';
import type { RequestContext } from '../request-context/index.js';
import type { InferPublicSchema, PublicSchema, StandardSchemaWithJSON, InferStandardSchemaOutput } from '../schema/index.js';
import type { OutputSchema } from '../stream/index.js';
import type { SchemaWithValidation } from '../stream/base/schema.js';
import type { ChunkType, WorkflowStreamEvent } from '../stream/types.js';
import type { Tool, ToolExecutionContext } from '../tools/index.js';
import type { DynamicArgument } from '../types/index.js';
import type { ExecutionEngine } from './execution-engine.js';
import type { WorkflowScheduleInput } from './scheduler/types.js';
import type { ConditionFunction, ExecuteFunction, ExecuteFunctionParams, LoopConditionFunction, Step } from './step.js';
export type OutputWriter<TChunk = any> = (chunk: TChunk, options?: {
    messageId?: string;
}) => Promise<void>;
/**
...
export type { ChunkType, WorkflowStreamEvent } from '../stream/types.js';
export type { MastraWorkflowStream } from '../stream/MastraWorkflowStream.js';
...
export type StepResult<P, R, S, T> = StepSuccess<P, R, S, T> | StepFailure<P, R, S, T> | StepSuspended<P, S, T> | StepRunning<P, R, S, T> | StepWaiting<P, R, S, T> | StepPaused<P, R, S, T> | StepSkipped<P, R, S, T>;
/**
 ...
 */
export type SerializedStepResult<P, R, S, T> = StepSuccess<P, R, S, T> | SerializedStepFailure<P, R, S, T> | StepFailure<P, R, S, T> | StepSuspended<P, S, T> | StepRunning<P, R, S, T> | StepWaiting<P, R, S, T> | StepPaused<P, R, S, T> | StepSkipped<P, R, S, T>;
export type TimeTravelContext<P, R, S, T> = Record<string, {
...
export type PathsToStringProps<T> = T extends z.ZodObject<infer V> ? PathsToStringProps<V> : T extends object ? {
    [K in keyof T]: T[K] extends object ? K extends string ? K | `${K}.${PathsToStringProps<T[K]>}` : never : K extends string ? K : never;
}[keyof T] : never;
export type ExtractSchemaType<T extends StandardSchemaWithJSON> = T extends StandardSchemaWithJSON ? InferStandardSchemaOutput<T> : never;
export type ExtractSchemaFromStep<TStep extends Step<any, any, any, any, any, any, any>, TKey extends 'inputSchema' | 'outputSchema'> = TStep[TKey];
export type VariableReference<TStep extends Step<string, any, any> = Step<string, any, any>, TVarPath extends PathsToStringProps<ExtractSchemaType<ExtractSchemaFromStep<TStep, 'outputSchema'>>> | '' | '.' = PathsToStringProps<ExtractSchemaType<ExtractSchemaFromStep<TStep, 'outputSchema'>>> | '' | '.'> = {
    step: TStep;
    path: TVarPath;
} | {
    value: any;
...
export type WorkflowRunStatus = 'running' | 'success' | 'failed' | 'tripwire' | 'suspended' | 'waiting' | 'pending' | 'canceled' | 'bailed' | 'paused' | 'skipped';
 ...
 */
export type WorkflowStateField = 'result' | 'error' | 'payload' | 'steps' | 'activeStepsPath' | 'serializedStepGraph' | 'suspendedPaths' | 'resumeLabels' | 'waitingPaths' | 'requestContext' | 'tracingContext';
export interface WorkflowRunState {
    ...
    } & Record<string, SerializedStepResult<any, any, any, any>>;
    serializedStepGraph: SerializedStepFlowEntry[];
...
export type SerializedStep<TEngineType = DefaultEngineType> = Pick<Step<any, any, any, any, any, any, TEngineType>, 'id' | 'description' | 'metadata'> & {
    component?: string;
 ...
 */
export type StepParams<TStepId extends string, TStateSchema extends PublicSchema<any> | undefined, TInputSchema extends PublicSchema<any>, TOutputSchema extends PublicSchema<any>, TResumeSchema extends PublicSchema<any> | undefined = undefined, TSuspendSchema extends PublicSchema<any> | undefined = undefined, TRequestContextSchema extends PublicSchema<any> | undefined = undefined> = {
    id: TStepId;
    description?: string;
    ...
    metadata?: StepMetadata;
    execute: ExecuteFunction<TStateSchema extends PublicSchema<any> ? InferPublicSchema<TStateSchema> : unknown, InferParsedPublicSchema<TInputSchema>, InferPublicSchema<TOutputSchema>, TResumeSchema extends PublicSchema<any> ? InferPublicSchema<TResumeSchema> : unknown, TSuspendSchema extends PublicSchema<any> ? InferPublicSchema<TSuspendSchema> : unknown, DefaultEngineType, TRequestContextSchema extends PublicSchema<any> ? InferPublicSchema<TRequestContextSchema> : unknown>;
};
/**
 * Legacy StepParams type for backward compatibility.
 * Use the schema-based StepParams for new code.
 */
export type StepParamsLegacy<TStepId extends string, TState, TStepInput, TStepOutput, TResume, TSuspend, TRequestContext extends Record<string, any> | unknown = unknown> = {
    id: TStepId;
    description?: string;
...
export type ToolStep<TSchemaIn, TSuspendSchema, TResumeSchema, TSchemaOut, TContext extends ToolExecutionContext<TSuspendSchema, TResumeSchema>> = Tool<TSchemaIn, TSchemaOut, TSuspendSchema, TResumeSchema, TContext> & {
...
export type WorkflowResult<TState, TInput, TOutput, TSteps extends Step<string, any, any, any, any, any>[]> = ({
    ...
    input: TInput;
    steps: {
        [K in keyof StepsRecord<TSteps>]: StepsRecord<TSteps>[K]['outputSchema'] extends undefined ? StepResult<unknown, unknown, unknown, unknown> : StepResult<InferStandardSchemaOutput<StepsRecord<TSteps>[K]['inputSchema']>, StepsRecord<TSteps>[K]['resumeSchema'] extends StandardSchemaWithJSON<any> ? InferStandardSchemaOutput<StepsRecord<TSteps>[K]['resumeSchema']> : unknown, StepsRecord<TSteps>[K]['suspendSchema'] extends StandardSchemaWithJSON<any> ? InferStandardSchemaOutput<StepsRecord<TSteps>[K]['suspendSchema']> : unknown, InferStandardSchemaOutput<StepsRecord<TSteps>[K]['outputSchema']>>;
    };
} & TracingProperties) | ({
    status: 'failed';
    input: TInput;
...
export type WorkflowStreamResult<TState, TInput, TOutput, TSteps extends Step<string, any, any>[]> = WorkflowResult<TState, TInput, TOutput, TSteps> | {
...
export type WorkflowConfig<TWorkflowId extends string, TState, TInput, TOutput, TSteps extends Step[], TRequestContext extends Record<string, any> | unknown = unknown> = {
    mastra?: Mastra;
    id: TWorkflowId;
    description?: string | undefined;
    metadata?: Record<string, unknown> | undefined;
...
export type CreateWorkflowParams<TWorkflowId extends string = string, TStateSchema extends PublicSchema<any> | undefined = undefined, TInputSchema extends PublicSchema<any> = PublicSchema<any>, TOutputSchema extends PublicSchema<any> = PublicSchema<any>, TSteps extends Step[] = Step[], TRequestContextSchema extends PublicSchema<any> | undefined = undefined> = {
    ...
    options?: WorkflowOptions;
    type?: WorkflowType;
    schedule?: WorkflowScheduleInput<NoInfer<InferSchemaOutput<TInputSchema>>, NoInfer<InferSchemaOutput<TStateSchema>>, NoInfer<InferSchemaOutput<TRequestContextSchema>>>;
};
/**
 ...
 */
export type SubsetOf<TStepState, TState> = unknown extends TState ? TStepState : 0 extends 1 & TStepState ? TStepState : unknown extends TStepState ? TStepState : TStepState extends infer TStepShape ? TState extends infer TStateShape ? keyof TStepShape extends keyof TStateShape ? {
    [K in keyof TStepShape]: TStepShape[K] extends TStateShape[K] ? TStepShape[K] : never;
} extends TStepShape ? TStepState : never : never : never : never;
/**
