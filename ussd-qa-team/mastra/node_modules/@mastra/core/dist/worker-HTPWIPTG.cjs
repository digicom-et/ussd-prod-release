...
// src/agent/heartbeat/worker.ts
var TOPIC_HEARTBEATS = "heartbeats";
var DEFAULT_GROUP = "mastra-heartbeats";
var HeartbeatWorker = class extends chunkCWKMGK5K_cjs.MastraWorker {
  name = "heartbeat";
  #config;
  #transport;
  #pushCb;
  ...
  async start() {
    ...
    this.#transport = new chunkCWKMGK5K_cjs.PullTransport({
...
async function executeHeartbeat(mastra, scheduleId, target, ctx = {}) {
  ...
  const hooks = mastra.__getHeartbeatHooks?.() ?? void 0;
...
function buildIfIdleOptions(ifIdle) {
  ...
  return {
    ...
    ...requestContext ? { streamOptions: { requestContext: new chunkPJIAL3WK_cjs.RequestContext(Object.entries(requestContext)) } } : {}
  };
}
...
function mergeProviderOptions(fromHook, heartbeatRunMeta) {
  const base = fromHook ?? {};
...
async function loadHeartbeatRef(mastra, scheduleId, target) {
  try {
...
exports.HeartbeatWorker = HeartbeatWorker;
exports.TOPIC_HEARTBEATS = TOPIC_HEARTBEATS;
exports.executeHeartbeat = executeHeartbeat;
//# sourceMappingURL=worker-HTPWIPTG.cjs.map
//# sourceMappingURL=worker-HTPWIPTG.cjs.map