...
var chunkVRIGWPHF_cjs = require('./chunk-VRIGWPHF.cjs');
...
function getSignalTagName(message) {
  const signal = message.content.metadata?.signal;
  if (signal && typeof signal === "object" && !Array.isArray(signal)) {
...
function isUserSignalType(type) {
...
function getSignalTagName2(message) {
  const signal = message.content.metadata?.signal;
  if (signal && typeof signal === "object" && !Array.isArray(signal)) {
    const tagName = signal.tagName;
    if (typeof tagName === "string") return tagName;
  }
  const type = getSignalType2(message);
  if (type === "user") return "user";
  if (type === "reactive") return message.type;
  return type;
}
function isUserSignalType2(type) {
  return type === "user" || type === "user-message";
}
...
function toSignalDataPart2(message) {
  ...
  const tagName = getSignalTagName2(message) ?? type;
  return {
    type: type === "user" ? "data-user-message" : "data-signal",
    data: {
      id: typeof signal.id === "string" ? signal.id : message.id,
...
function transformToolStateDataForDisplay(data, phase, enabled = true) {
  ...
  const transformedArgs = phase === "approval" ? hasTransformedToolPayload(argsTransform) ? argsTransform.transformed : hasTransformedToolPayload(inputTransform) ? inputTransform.transformed : void 0 : hasTransformedToolPayload(inputTransform) ? inputTransform.transformed : hasTransformedToolPayload(argsTransform) ? argsTransform.transformed : void 0;
  const transformedSuspendPayload = phase === "suspend" && hasTransformedToolPayload(argsTransform) ? argsTransform.transformed : void 0;
  return {
    ...stateData,
...
var AIV5Adapter = class {
  ...
  static toUIMessage(dbMsg, options) {
    const signalType = dbMsg.role === "signal" ? getSignalType2(dbMsg) : void 0;
    const isUserMessageSignal = isUserSignalType2(signalType);
    const transformToolPayloads = options?.transformToolPayloads ?? true;
    const parts = [];
    const metadata = { ...dbMsg.content.metadata || {} };
    if (dbMsg.role === "signal" && !isUserMessageSignal) {
      parts.push(toSignalDataPart2(dbMsg));
    }
        ...
        if (part.type === "tool-invocation" && part.toolInvocation) {
          ...
          if (inv.state === "result") {
            parts.push({
              ...
              input: getDisplayTransform2(part.providerMetadata, "input-available", inv.args, transformToolPayloads),
              output: getDisplayTransform2(
                part.providerMetadata,
                "output-available",
                getDisplayTransform2(part.providerMetadata, "error", inv.result, transformToolPayloads),
                transformToolPayloads
              ),
              state: "output-available",
            ...
            parts.push({
              ...
              toolCallId: inv.toolCallId,
              input: getDisplayTransform2(part.providerMetadata, "input-available", inv.args, transformToolPayloads),
              errorText: getDisplayTransform2(
                part.providerMetadata,
                "error",
                inv.errorText || "",
                transformToolPayloads
    ...
    const existingToolStateDataPartIds = new Set(
      parts.filter(
        ...
        return typeof data?.toolCallId === "string" ? data.toolCallId : void 0;
      }).filter((toolCallId) => typeof toolCallId === "string")
    );
    const insertToolStateDataPart = (toolCallId, toolStateDataPart) => {
      const toolPartIndex = parts.findIndex(
        (part) => part.type.startsWith("tool-") && part.toolCallId === toolCallId
      );
      if (toolPartIndex === -1) {
    ...
    if (suspendedTools && typeof suspendedTools === "object") {
      for (const suspendedTool of Object.values(suspendedTools)) {
        ...
        const toolCallId = "toolCallId" in suspendedTool ? suspendedTool.toolCallId : void 0;
    ...
    if (pendingToolApprovals && typeof pendingToolApprovals === "object") {
      for (const pendingToolApproval of Object.values(pendingToolApprovals)) {
        ...
        const toolCallId = "toolCallId" in pendingToolApproval ? pendingToolApproval.toolCallId : void 0;
        if (typeof toolCallId !== "string" || existingToolStateDataPartIds.has(toolCallId)) {
          continue;
        }
        insertToolStateDataPart(toolCallId, {
  ...
  static fromUIMessage(uiMsg) {
    ...
    const toolInvocationParts = parts.filter((p) => chunkVRIGWPHF_cjs.isToolUIPart(p));
    ...
    const v2Parts = parts.map((p) => {
      if (chunkVRIGWPHF_cjs.isToolUIPart(p)) {
        const toolName = getToolName(p);
  ...
  static fromModelMessage(modelMsg, _messageSource, context = {}) {
    ...
    for (const part of content) {
      if (part.type === "text") {
        ...
        const recoveredArgs = context.dbMessages ? findToolCallArgs(context.dbMessages, toolResultPart.toolCallId) : void 0;
        const fallbackArgs = recoveredArgs && Object.keys(recoveredArgs).length > 0 ? recoveredArgs : toolResultPart.input ?? {};
        if (matchingCall) {
...
function getDisplayTransform3(providerMetadata, phase, fallback) {
  const transform = getTransformedToolPayload(providerMetadata, "display", phase);
  return hasTransformedToolPayload(transform) ? transform.transformed : fallback;
}
...
function normalizeToolResult(output) {
  return typeof output === "object" && output && "value" in output ? output.value : output;
}
...
function createToolInvocationPartFromUIPart(part) {
  const base = {
    toolCallId: part.toolCallId,
    toolName: getToolNameFromUIPart(part),
    args: normalizeToolArgs(part.input),
    approval: "approval" in part ? toMastraApproval(part.approval) : void 0,
...
function createLegacyToolInvocations(parts) {
  const toolInvocations = [];
  for (const part of parts) {
  ...
  return toolInvocations.length > 0 ? toolInvocations : void 0;
}
var AIV6Adapter = class _AIV6Adapter {
  static toUIMessage(dbMsg) {
    ...
    if (dbMsg.role === "signal" && v5Message.role !== "user") {
      return {
        ...
        role: "system",
        metadata: Object.keys(metadata).length > 0 ? metadata : void 0,
        parts: v5Message.parts.map((part) => _AIV6Adapter.toUIPartFromV5(part))
      };
    }
    ...
    if (!hasToolInvocationParts || !hasReasoningParts || !hasFileParts || !hasTextParts) {
      for (const part of v5Message.parts) {
        if (chunkVRIGWPHF_cjs.isToolUIPart(part)) {
  ...
  static fromUIMessage(uiMsg) {
    const compatibleParts = uiMsg.parts.filter((part) => {
      ...
      if (chunk4SCYX53T_cjs.isToolUIPart(part)) return false;
      return true;
    });
    ...
    for (const part of uiMsg.parts) {
      ...
      if (!chunk4SCYX53T_cjs.isToolUIPart(part)) {
      ...
      switch (part.toolInvocation.state) {
        case "partial-call":
          return {
            ...
            input: getDisplayTransform3(part.providerMetadata, "input-available", part.toolInvocation.args)
          };
        case "call":
          return {
            ...base,
          ...
          return withOptionalFields(
            {
              ...
              state: "output-error",
              input: getDisplayTransform3(part.providerMetadata, "input-available", part.toolInvocation.args),
              errorText: getDisplayTransform3(
                part.providerMetadata,
                "error",
          ...
          return withOptionalFields(
            {
              ...
              state: "output-available",
              input: getDisplayTransform3(part.providerMetadata, "input-available", part.toolInvocation.args),
              output: getDisplayTransform3(
                part.providerMetadata,
                "output-available",
                getDisplayTransform3(part.providerMetadata, "error", part.toolInvocation.result)
              )
            },
            {
...
// src/agent/message-list/cache/stable-stringify.ts
...
function appendResponseProviderItemKeys(cacheKey, ...providerSources) {
  const itemKeys = new Set(
    providerSources.flatMap((source) => getResponseProviderItemKeys(source))
  );
...
var CacheKeyGenerator = class _CacheKeyGenerator {
  ...
  static fromAIV4Part(part) {
    ...
    if (part.type === "text") {
      cacheKey += part.text;
      cacheKey = appendResponseProviderItemKeys(cacheKey, part.providerMetadata);
    }
    if (part.type === "tool-invocation") {
  ...
  static fromAIV5Parts(parts) {
    ...
    for (const part of parts) {
      ...
      if (part.type === "text") {
        key += part.text;
        key = appendResponseProviderItemKeys(key, part.providerMetadata);
      }
      if (chunkVRIGWPHF_cjs.isToolUIPart(part) || part.type === "dynamic-tool") {
        key += part.toolCallId;
        key += part.state;
      }
      if (part.type === "reasoning") {
  ...
  static fromAIV5ModelMessageContent(content) {
    ...
    for (const part of content) {
      ...
      if (part.type === "text") {
        key += part.text.length;
        key = appendResponseProviderItemKeys(key, part.providerOptions);
      }
      if (part.type === "reasoning") {
        key += part.text.length;
        key = appendResponseProviderItemKeys(key, part.providerOptions);
  ...
  return prompt.map((message) => {
    ...
    const content = message.content.map((part) => {
      ...
      const value = output.value.map((item) => {
        ...
        return mediaType.startsWith(`image/`) ? { type: `image-data`, data: contentPart.data, mediaType } : { type: `file-data`, data: contentPart.data, mediaType };
      });
...
function hydrateMastraDBMessageFields(message, context, messageSource) {
  if (!message.id) {
    message.id = context.newMessageId();
  }
  if (message.createdAt === void 0 || message.createdAt === null) {
...
function isTextMergePassThroughPart(part) {
  ...
  for (const part of parts) {
    ...
    for (let index = result.length - 1; index >= 0; index--) {
      ...
      if (previous.type === "text") {
        const previousTextPart = previous;
...
function sanitizeV5UIMessages(messages, filterIncompleteToolCalls = false) {
  ...
  const getSafeParts = (m, assistantTurnStillOpen) => m.parts.filter((p) => {
    if (typeof p.type === "string" && p.type.startsWith("data-")) {
      return false;
    }
    ...
    if (!chunkVRIGWPHF_cjs.isToolUIPart(p)) return true;
  ...
  const msgs = messages.map((m, idx) => {
    ...
    const sanitized = {
      ...
      parts: mergedParts.map((part) => {
        if (chunkVRIGWPHF_cjs.isToolUIPart(part) && part.state === "output-available") {
          return {
            ...part,
            output: (() => {
              const o = part.output;
...
function addStartStepPartsForAIV5(messages) {
  for (const message of messages) {
    ...
    for (const [index, part] of message.parts.entries()) {
      if (!chunkVRIGWPHF_cjs.isToolUIPart(part)) continue;
      const nextPart = message.parts.at(index + 1);
      if (nextPart && nextPart.type !== `step-start` && !chunkVRIGWPHF_cjs.isToolUIPart(nextPart)) {
        message.parts.splice(index + 1, 0, { type: "step-start" });
      }
      if (nextPart && chunkVRIGWPHF_cjs.isToolUIPart(nextPart) && !part.providerExecuted && nextPart.providerExecuted && (nextPart.state === "output-available" || nextPart.state === "output-error")) {
        message.parts.splice(index + 1, 0, { type: "step-start" });
      }
    }
  }
...
function aiV4UIMessagesToAIV4CoreMessages(messages) {
  return chunkI4YELKDU_cjs.convertToCoreMessages(sanitizeAIV4UIMessages(messages));
}
function convertMcpContentToolResultOutput(output) {
  ...
  if (!hasValidMultimodal) return void 0;
  const value = content.map((part) => {
    ...
    switch (typedPart.type) {
      ...
      case "image":
        return typeof typedPart.data === "string" ? { type: "image-data", data: typedPart.data, mediaType: String(typedPart.mimeType ?? "image/png") } : { type: "text", text: JSON.stringify(typedPart) };
      case "audio":
        return typeof typedPart.data === "string" ? { type: "file-data", data: typedPart.data, mediaType: String(typedPart.mimeType ?? "audio/wav") } : { type: "text", text: JSON.stringify(typedPart) };
      default:
        return { type: "text", text: JSON.stringify(typedPart) };
    }
  }).filter(Boolean);
  return value.length > 0 ? { type: "content", value } : void 0;
}
function collectRawToolResultOutputs(dbMessages) {
...
function isDefaultToolResultOutput(output, rawOutput) {
  if (!output || typeof output !== "object") return false;
  const typedOutput = output;
  if (typedOutput.type !== "json") return false;
  return JSON.stringify(typedOutput.value) === JSON.stringify(rawOutput);
}
function applyMcpContentToolResultOutputs(modelMessages, dbMessages) {
...
function aiV5UIMessagesToAIV5ModelMessages(messages, dbMessages, filterIncompleteToolCalls = false) {
  ...
  for (const uiMsg of preprocessed) {
    const produced = chunkVRIGWPHF_cjs.convertToModelMessages([uiMsg]);
    if (produced.length === 0) continue;
    const providerMetadata = uiMsg.metadata && typeof uiMsg.metadata === "object" && "providerMetadata" in uiMsg.metadata ? uiMsg.metadata.providerMetadata : void 0;
    if (providerMetadata) {
      let target = -1;
      ...
      if (target !== -1) {
        produced[target] = { ...produced[target], providerOptions: providerMetadata };
      }
    }
...
var StepContentExtractor = class _StepContentExtractor {
  ...
  static convertPartsToContent(parts, stepId, stepContentFn) {
    ...
    const modelMessages = chunkVRIGWPHF_cjs.convertToModelMessages(sanitizeV5UIMessages(stepUiMessages));
...
var MessageMerger = class _MessageMerger {
  ...
  static shouldMerge(latestMessage, incomingMessage, messageSource, isLatestFromMemory, agentNetworkAppend = false) {
    ...
    if (latestMessage.content?.metadata?.mastra?.responseBoundary) {
      return false;
    }
  ...
  static pushNewPart({
    ...
    if (latestPartCount < newPartCount) {
      ...
      const stepStartPart = previousStepStart?.model ? stampPart({
        type: "step-start",
...
var downloadFromUrl = async ({ url, downloadRetries }) => {
  ...
  try {
    const response = await chunkPAMAGEYY_cjs.fetchWithRetry(
      urlText,
      {
        method: "GET"
      },
      downloadRetries,
      {
        shouldRetryResponse: (response2) => response2.status >= 500
      }
    );
    if (!response.ok) {
      throw new chunkXSOONORA_cjs.MastraError({
...
function mergeSignalDataParts(messages) {
  ...
  for (let idx = 0; idx < messages.length; idx++) {
    ...
    if (prev && prev.role === "assistant") {
      ...
      messages[idx + 1] = { ...next, parts: [...message.parts, ...next.parts] };
    } else {
      result.push({ ...message, role: "assistant" });
    }
  }
  return result;
}
...
function mergeBackgroundTasks(existingBgTasks, incomingBgTasks) {
  if (!existingBgTasks && !incomingBgTasks) {
    return void 0;
  }
  const merged = { ...existingBgTasks ?? {} };
  for (const [toolCallId, incomingTask] of Object.entries(incomingBgTasks ?? {})) {
    const existingTask = merged[toolCallId];
    merged[toolCallId] = isPlainRecord(existingTask) && isPlainRecord(incomingTask) ? { ...existingTask, ...incomingTask } : incomingTask;
  }
  return merged;
}
var MessageList = class {
  ...
  addSignal(signal, options) {
    const source = options?.source ?? "input";
    const createdAt = this.generateCreatedAt(source, /* @__PURE__ */ new Date());
    ...
    this.addOne(signalForTranscript.toDBMessage(this.memoryInfo ?? void 0), source);
    return signalForTranscript;
  }
  add(messages, messageSource, options = {}) {
    ...
    for (const message of messageArray) {
      ...
      const messageInput = isCreatedAgentSignal(message) ? message.toDBMessage(this.memoryInfo ?? void 0) : typeof message === `string` ? {
        role: "user",
        content: message
      } : message;
  ...
  convertSignalForModelPrompt(message) {
    ...
    return [
      inputToMastraDBMessage(promptMessage, "input", {
        memoryInfo: this.memoryInfo,
  ...
  all = {
    ...
    aiV5: {
      ...
      ui: () => this.toAIV5UIMessages(this.all.db()),
    ...
    aiV6: {
      ...
      llmPrompt: async (options) => aiV5PromptToAIV6Prompt(await this.all.aiV5.llmPrompt(options))
    },
    /* @deprecated use list.get.all.aiV4.prompt() instead */
    prompt: () => this.all.aiV4.prompt(),
    /* @deprecated use list.get.all.aiV4.ui() */
    ui: () => this.toAIV4UIMessages(this.all.db()),
    /* @deprecated use list.get.all.aiV4.core() */
    core: () => aiV4UIMessagesToAIV4CoreMessages(this.toAIV4UIMessages(this.all.db(), { transformToolPayloads: false })),
    aiV4: {
      ui: () => this.toAIV4UIMessages(this.all.db()),
      core: () => aiV4UIMessagesToAIV4CoreMessages(
  ...
  remembered = {
    ...
    aiV5: {
      model: () => aiV5UIMessagesToAIV5ModelMessages(
        this.toAIV5UIMessages(this.remembered.db(), { transformToolPayloads: false }),
        this.messages
      ),
      ui: () => this.toAIV5UIMessages(this.remembered.db())
    },
    aiV6: {
      ui: () => this.toAIV6UIMessages(this.remembered.db())
    },
    /* @deprecated use list.get.remembered.aiV4.ui() */
    ui: () => this.toAIV4UIMessages(this.remembered.db()),
    /* @deprecated use list.get.remembered.aiV4.core() */
    core: () => aiV4UIMessagesToAIV4CoreMessages(this.toAIV4UIMessages(this.remembered.db(), { transformToolPayloads: false })),
    aiV4: {
      ui: () => this.toAIV4UIMessages(this.remembered.db()),
      core: () => aiV4UIMessagesToAIV4CoreMessages(this.toAIV4UIMessages(this.remembered.db(), { transformToolPayloads: false }))
    }
  ...
  rememberedPersisted = {
    ...
    aiV5: {
      ...
      ui: () => this.toAIV5UIMessages(this.rememberedPersisted.db())
    },
    aiV6: {
  ...
  input = {
    ...
    aiV5: {
      ...
      ui: () => this.toAIV5UIMessages(this.input.db())
    },
    aiV6: {
    ...
    core: () => aiV4UIMessagesToAIV4CoreMessages(this.toAIV4UIMessages(this.input.db(), { transformToolPayloads: false })),
    aiV4: {
      ui: () => this.toAIV4UIMessages(this.input.db()),
      core: () => aiV4UIMessagesToAIV4CoreMessages(this.toAIV4UIMessages(this.input.db(), { transformToolPayloads: false }))
    }
  ...
  inputPersisted = {
    ...
    aiV5: {
      ...
      ui: () => this.toAIV5UIMessages(this.inputPersisted.db())
    },
    aiV6: {
      ui: () => this.toAIV6UIMessages(this.inputPersisted.db())
    },
    /* @deprecated use list.getPersisted.input.aiV4.ui() */
    ui: () => this.toAIV4UIMessages(this.inputPersisted.db()),
  ...
  response = {
    ...
    aiV5: {
      ui: () => this.toAIV5UIMessages(this.response.db()),
      model: () => aiV5UIMessagesToAIV5ModelMessages(
    ...
    aiV4: {
      ui: () => this.toAIV4UIMessages(this.response.db()),
      core: () => aiV4UIMessagesToAIV4CoreMessages(this.toAIV4UIMessages(this.response.db(), { transformToolPayloads: false }))
    }
  };
  responsePersisted = {
    ...
    aiV5: {
      ...
      ui: () => this.toAIV5UIMessages(this.responsePersisted.db())
    },
    aiV6: {
      ui: () => this.toAIV6UIMessages(this.responsePersisted.db())
    },
    /* @deprecated use list.getPersisted.response.aiV4.ui() */
    ui: () => this.toAIV4UIMessages(this.responsePersisted.db()),
    aiV4: {
      ui: () => this.toAIV4UIMessages(this.responsePersisted.db()),
      core: () => aiV4UIMessagesToAIV4CoreMessages(
  ...
  transformToolStateDataForTranscript(data, phase) {
    if (!data || typeof data !== "object") {
      return data;
    }
    const stateData = data;
    ...
    const transformedArgs = phase === "approval" ? hasTransformedToolPayload(phaseTransform) ? phaseTransform.transformed : hasTransformedToolPayload(inputTransform) ? inputTransform.transformed : void 0 : hasTransformedToolPayload(inputTransform) ? inputTransform.transformed : hasTransformedToolPayload(phaseTransform) ? phaseTransform.transformed : void 0;
    const transformedSuspendPayload = phase === "suspend" && hasTransformedToolPayload(phaseTransform) ? phaseTransform.transformed : void 0;
    return {
      ...stateData,
      ...transformedArgs !== void 0 ? { args: transformedArgs } : {},
      ...transformedSuspendPayload !== void 0 ? { suspendPayload: transformedSuspendPayload } : {}
  ...
  transformMessageForTranscript(message) {
    if (message.content?.format !== 2 || !message.content.parts) {
      return message;
    }
    let changed = false;
    const transformedByToolCallId = /* @__PURE__ */ new Map();
    const parts = message.content.parts.map((part) => {
      if (part.type === "tool-invocation" && part.toolInvocation) {
        ...
        const outputTransform = part.toolInvocation.state === "result" ? getTransformedToolPayload(part.providerMetadata, "transcript", "output-available") ?? getTransformedToolPayload(part.providerMetadata, "transcript", "error") : part.toolInvocation.state === "output-error" ? getTransformedToolPayload(part.providerMetadata, "transcript", "error") : void 0;
        if (!inputTransform && !outputTransform) {
          return part;
        }
        changed = true;
        const transformedArgs = hasTransformedToolPayload(inputTransform) ? inputTransform.transformed : part.toolInvocation.args;
        const transformedResult = part.toolInvocation.state === "result" ? hasTransformedToolPayload(outputTransform) ? outputTransform.transformed : part.toolInvocation.result : void 0;
        const transformedErrorText = part.toolInvocation.state === "output-error" ? hasTransformedToolPayload(outputTransform) ? outputTransform.transformed : part.toolInvocation.errorText : void 0;
    ...
    const toolInvocations = message.content.toolInvocations?.map((invocation) => {
      ...
      return {
        ...invocation,
        ...transformed.args !== void 0 ? { args: transformed.args } : {},
        ...invocation.state === "result" && transformed.result !== void 0 ? { result: transformed.result } : {},
        ...invocationState === "output-error" && transformed.errorText !== void 0 ? { errorText: transformed.errorText } : {}
      };
    });
    const metadata = message.content.metadata && typeof message.content.metadata === "object" ? { ...message.content.metadata } : message.content.metadata;
    if (metadata && typeof metadata === "object") {
      for (const [key, phase] of [
        ...
        metadata[key] = Object.fromEntries(
          Object.entries(toolStates).map(([toolName, state]) => [
            toolName,
   ...
   */
  updateToolInvocation(inputPart, metadata) {
    ...
    for (let m = this.messages.length - 1; m >= 0; m--) {
      ...
      for (let i = 0; i < msg.content.parts.length; i++) {
        ...
        if (part?.type === "tool-invocation" && part.toolInvocation?.toolCallId === toolCallId) {
          ...
          const mergedProviderMetadata = originalPart.providerMetadata !== void 0 || inputPartWithMeta.providerMetadata !== void 0 ? {
            ...originalPart.providerMetadata ?? {},
            ...inputPartWithMeta.providerMetadata ?? {}
          } : void 0;
          msg.content.parts[i] = {
            ...inputPart,
            toolInvocation: {
            ...
            ...mergedProviderMetadata !== void 0 ? { providerMetadata: mergedProviderMetadata } : {}
          };
          this.lastCreatedAt = Math.max(this.lastCreatedAt || 0, Date.now());
          this.updateLastCreatedAt(msg);
          const existingMeta = msg.content.metadata ?? {};
          const incomingMeta = metadata ?? {};
          const existingBgTasks = existingMeta.backgroundTasks;
          const incomingBgTasks = incomingMeta.backgroundTasks;
  ...
  updateMessageMetadataByToolCallId(toolCallId, metadata) {
    if (!toolCallId) {
      return false;
    }
    for (let m = this.messages.length - 1; m >= 0; m--) {
  ...
  markResponseMessageBoundary(messageId) {
    const message = messageId ? this.messages.find((message2) => message2.id === messageId) : [...this.messages].reverse().find((message2) => message2.role === "assistant");
  ...
  addOne(message, messageSource, options = {}) {
    ...
    const signalMetadata = messageV2.role === "signal" ? messageV2.content.metadata?.signal : void 0;
    ...
    const latestSealedIndex = this.messages.findLastIndex((message2) => MessageMerger.isSealed(message2));
    ...
    const shouldMerge = options.merge !== false && latestMessageIsAfterSealedBoundary && !hasSealedReplacementTarget && MessageMerger.shouldMerge(latestMessage, messageV2, messageSource, isLatestFromMemory, this._agentNetworkAppend);
    if (shouldMerge && latestMessage) {
      ...
      if (shouldReplace && existingMessage) {
        ...
        if (MessageMerger.isSealed(existingMessage)) {
          ...
          const shouldMergeIntoExisting = options.merge !== false && MessageMerger.shouldMerge(
            existingMessage,
            messageV2,
            messageSource,
...
exports.AIV6Adapter = AIV6Adapter;
...
exports.createMessageSignal = createMessageSignal;
exports.createSignal = createSignal;
exports.dataPartToSignal = dataPartToSignal;
...
exports.getResponseProviderItemId = getResponseProviderItemId;
exports.getResponseProviderItemIdFromPart = getResponseProviderItemIdFromPart;
exports.getResponseProviderItemIds = getResponseProviderItemIds;
exports.getResponseProviderItemKey = getResponseProviderItemKey;
exports.getResponseProviderItemKeys = getResponseProviderItemKeys;
exports.getTransformedToolPayload = getTransformedToolPayload;
exports.hasOpenAIReasoningItemId = hasOpenAIReasoningItemId;
exports.hasResponseProviderItemId = hasResponseProviderItemId;
exports.hasTransformedToolPayload = hasTransformedToolPayload;
exports.isCreatedAgentSignal = isCreatedAgentSignal;
exports.isMastraSignalMessage = isMastraSignalMessage;
exports.mastraDBMessageToSignal = mastraDBMessageToSignal;
exports.messagesAreEqual = messagesAreEqual;
exports.normalizeToolPayloadTransformPolicy = normalizeToolPayloadTransformPolicy;
exports.resolveDeliveryAttributes = resolveDeliveryAttributes;
exports.sanitizeOrphanedToolPairs = sanitizeOrphanedToolPairs;
exports.sanitizeToolName = sanitizeToolName;
exports.signalToDataPartFormat = signalToDataPartFormat;
exports.signalToMastraDBMessage = signalToMastraDBMessage;
exports.signalToMessage = signalToMessage;
exports.signalToXmlMarkup = signalToXmlMarkup;
exports.stableStringify = stableStringify;
exports.transformToolPayloadForTargets = transformToolPayloadForTargets;
exports.withToolPayloadTransformMetadata = withToolPayloadTransformMetadata;
exports.withToolPayloadTransformProviderMetadata = withToolPayloadTransformProviderMetadata;
//# sourceMappingURL=chunk-DC5C6QMM.cjs.map
//# sourceMappingURL=chunk-DC5C6QMM.cjs.map