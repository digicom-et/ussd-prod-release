import { CachingPubSub } from '../../chunk-V7JI3TXI.js';
import { SaveQueueManager, createStep, mergeProviderOptions, PrepareStepProcessor, execute, findProviderToolByName, createWorkflow, runScorer, Agent, runStreamCompletionScorers, formatStreamCompletionFeedback } from '../../chunk-PQ5PN4TW.js';
import { PUBSUB_SYMBOL } from '../../chunk-2QXNHEDL.js';
import { MastraModelOutput, isSupportedLanguageModel, ProcessorRunner, safeClose, safeEnqueue } from '../../chunk-YNS26J6E.js';
import { resolveModelConfig } from '../../chunk-ORABIARW.js';
import { EventEmitterPubSub } from '../../chunk-2S76KRB5.js';
import { InMemoryServerCache } from '../../chunk-366PLPDH.js';
import { deepMerge } from '../../chunk-M2TW4P2L.js';
import { getNeedsApprovalFn } from '../../chunk-VLJMDX6T.js';
import { createObservabilityContext } from '../../chunk-JPUBRZLW.js';
import { getOrCreateSpan, EntityType, getStepAvailableToolNames } from '../../chunk-DEQCJWZZ.js';
import { resolveBackgroundConfig, createBackgroundTask } from '../../chunk-GCIWBMEH.js';
import { MessageList, normalizeToolPayloadTransformPolicy, transformToolPayloadForTargets, withToolPayloadTransformMetadata } from '../../chunk-FDU3KNKR.js';
...
async function runWithIdleWrapper(agent, streamOptions, deps, firstTurn) {
  ...
  if (!deps.bgManager || !scope) {
    return firstTurn(restStreamOptions);
  }
...
async function prepareForDurableExecution(options) {
  ...
  if (inputProcessors.length > 0) {
    try {
      const { ProcessorRunner: ProcessorRunner2 } = await import('../../runner-2DAIS6LW.js');
  ...
  const toolPayloadTransform = normalizeToolPayloadTransformPolicy(execOptions?.transform) ?? typedAgent.getToolPayloadTransform?.() ?? normalizeToolPayloadTransformPolicy(
...
async function emitAbortEvent(pubsub, runId, data) {
  await pubsub.publish(AGENT_STREAM_TOPIC(runId), {
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
          try {
            ...
            if (modelSupportedUrls) {
              resolvedSupportedUrls = typeof modelSupportedUrls.then === "function" ? await modelSupportedUrls : modelSupportedUrls;
            }
...
function createDurableToolCallStep() {
  return createStep({
    ...
    execute: async (params) => {
      ...
      let mastraTools;
      if (!tool) {
        tool = findProviderToolByName(registryEntry?.tools, toolName);
      }
      if (!tool) {
      ...
      const toolKey = registryEntry?.tools?.[toolName] ? toolName : Object.entries(registryEntry?.tools ?? {}).find(([, registeredTool]) => registeredTool === tool)?.[0] ?? Object.entries(mastraTools ?? {}).find(([, registeredTool]) => registeredTool === tool)?.[0];
  ...
  return createStep({
    ...
    execute: async ({ inputData, mastra, requestContext }) => {
      ...
      if (llmOutput.stepSpanData) {
        try {
          const observability = mastra?.observability?.getSelectedInstance({ requestContext });
          const stepSpan = observability?.rebuildSpan(llmOutput.stepSpanData);
...
function createDurableAgenticWorkflow(options) {
  ...
  const singleIterationWorkflow = createWorkflow({
    ...
    options: {
      shouldPersistSnapshot: (params) => {
        return params.workflowStatus === "pending" || params.workflowStatus === "paused" || params.workflowStatus === "suspended";
      },
  ...
  ).then(isTaskCompleteStep).commit();
...
var DurableAgent = class extends Agent {
  ...
  async stream(messages, options) {
    ...
    const {
      ...
      onIterationComplete: options?.onIterationComplete ? async (data) => {
        await options.onIterationComplete?.(data);
      } : void 0,
      closeOnSuspend: options?.[CLOSE_ON_SUSPEND] === true
    });
    const workflowExecution = ready.then(() => this.executeWorkflow(runId, workflowInput)).catch((error) => {
...
export { AGENT_STREAM_TOPIC, AgentStreamEventTypes, DurableAgent, DurableAgentDefaults, DurableStepIds, EventedAgent, ExtendedRunRegistry, RunRegistry, accumulatedUsageSchema, baseDurableAgenticInputSchema, baseIterationStateSchema, buildStepRecord, calculateAccumulatedUsage, createBaseIterationStateUpdate, createDurableAgent, createDurableAgentStream, createDurableAgenticWorkflow, createDurableBackgroundTaskCheckStep, createDurableLLMExecutionStep, createDurableLLMMappingStep, createDurableToolCallStep, createEventedAgent, createWorkflowInput, durableAgenticOutputSchema, emitChunkEvent, emitErrorEvent, emitFinishEvent, emitStepFinishEvent, emitStepStartEvent, emitSuspendedEvent, executeDurableToolCalls, globalRunRegistry, isDurableAgent, isEventedAgent, isEventedAgentClass, isLocalDurableAgent, modelConfigSchema, modelListEntrySchema, prepareForDurableExecution, resolveInternalState, resolveModel, resolveRuntimeDependencies, resolveTool, runDurableStreamUntilIdle, runResumeDurableStreamUntilIdle, serializeDurableOptions, serializeDurableState, serializeModelConfig, serializeToolsMetadata, toolRequiresApproval };
