...
function getDisplayTransform(metadata, phase, fallback) {
  const transform = chunkDC5C6QMM_cjs.getTransformedToolPayload(metadata, "display", phase);
  return chunkDC5C6QMM_cjs.hasTransformedToolPayload(transform) ? transform.transformed : fallback;
}
...
function toUserSignalMessage(payload) {
  ...
  const signal = chunkDC5C6QMM_cjs.createSignal({
    id,
    type: "user",
    tagName: "user",
...
function describeServerSideFallback(providerMetadata) {
  const fallback = chunkQJ3FDYCK_cjs.getServerSideFallbackInfo(providerMetadata);
  if (!fallback) {
    return void 0;
  }
...
var SessionRunEngine = class {
  ...
  async processStreamChunk(state, chunk, requestContext) {
    ...
    switch (chunk.type) {
      ...
      case "tool-call-delta": {
        ...
        const transform = chunkDC5C6QMM_cjs.getTransformedToolPayload(chunk.metadata, "display", "input-delta");
        if (!transform?.suppress) {
          this.#session.emit({
            type: "tool_input_delta",
            toolCallId,
            argsTextDelta: chunkDC5C6QMM_cjs.hasTransformedToolPayload(transform) ? transform.transformed : argsTextDelta,
            toolName
          });
        }
        break;
      ...
      case "tool-call-approval": {
        ...
        const approvalTransform = chunkDC5C6QMM_cjs.getTransformedToolPayload(chunk.metadata, "display", "approval");
        const toolArgs = chunkDC5C6QMM_cjs.hasTransformedToolPayload(approvalTransform) ? approvalTransform.transformed : getDisplayTransform(chunk.metadata, "input-available", chunk.payload.args);
      ...
      case "error": {
        const streamError = chunkXSOONORA_cjs.getErrorFromUnknown(chunk.payload.error);
  ...
  async handleSubscribedStreamError(error) {
    if (error instanceof Error && error.name === "AbortError") {
      ...
      this.#session.emit({ type: "error", error: chunkXSOONORA_cjs.getErrorFromUnknown(error) });
...
var SessionThread = class {
  ...
  async loadMetadata() {
    ...
    try {
      ...
      const modeModelKey2 = `modeModelId_${currentModeId}`;
      if (meta?.[modeModelKey2]) {
        session.model.set({ modelId: meta[modeModelKey2] });
      } else {
        const currentMode = session.mode.resolve();
        if (currentMode.defaultModelId) {
          session.model.set({ modelId: currentMode.defaultModelId });
        } else if (meta?.currentModelId) {
          session.model.set({ modelId: meta.currentModelId });
        }
      }
...
var SessionSuspensions = class {
  ...
  resolveToolCallId(toolCallId) {
    if (toolCallId) {
      return this.#pending.has(toolCallId) ? toolCallId : void 0;
    }
...
var SessionOMRole = class {
  ...
  resolvedModel() {
    ...
    return new chunk2PXTHATK_cjs.ModelRouterLanguageModel(modelId, this.#gateways);
  }
...
var SessionPermissions = class {
  ...
  getRules() {
    const rules = this.#getState?.().permissionRules;
...
var SessionState = class {
  ...
  constructor({ initialState, stateSchema }, bus) {
    this.#schema = stateSchema ? chunkXB4FLS7A_cjs.toStandardSchema(stateSchema) : void 0;
    this.#state = {
  ...
  async apply(updates) {
    ...
    this.#bus.emit({ type: "state_changed", state: this.get(), changedKeys });
  }
  set(updates) {
  ...
  apply(event) {
    ...
    switch (event.type) {
      ...
      case "tool_update": {
        ...
        if (tool) {
          tool.partialResult = typeof event.partialResult === "string" ? event.partialResult : chunkBP67LMWW_cjs.safeStringify(event.partialResult);
        }
        break;
      }
      case "tool_end": {
    ...
    for (const event of this.#lastWorkspaceEvents) {
      try {
        ...
        if (result && typeof result === "object" && "catch" in result) {
          result.catch((err) => console.error("Error in session event listener:", err));
        }
      } catch (err) {
...
var Session = class {
  ...
  constructor({
    ...
    if (!workspace || !(workspace instanceof chunkQJ3FDYCK_cjs.Workspace)) {
...
${fence}` };
  ...
  sendSignal(input) {
    ...
    const accepted = Promise.resolve().then(async () => {
      ...
      if (submittedRunId && submittedActiveRunId && submittedIsRunning) {
        ...
        return { accepted: true, runId: await settleRunId(result2) };
      }
      ...
      return { accepted: true, runId: void 0 };
    });
  ...
  async respondToToolSuspension({
    ...
    try {
      ...
      const err = chunkXSOONORA_cjs.getErrorFromUnknown(error);
  ...
  async resumeToolCall({
    ...
    const resumedSubscriptionBoundary = this.createSubscribedResumeBoundaryWaiter(
      suspension.toolName === "submit_plan" ? toolCallId : void 0
    );
    try {
      const resourceId = this.identity.getResourceId();
  ...
  addUsage(stepUsage) {
    ...
    addOptionalUsageField2(this.#tokenUsage, "reasoningTokens", stepUsage.reasoningTokens);
    addOptionalUsageField2(this.#tokenUsage, "cachedInputTokens", stepUsage.cachedInputTokens);
    addOptionalUsageField2(this.#tokenUsage, "cacheCreationInputTokens", stepUsage.cacheCreationInputTokens);
...
function createSubagentTool(opts) {
  ...
  return chunkDZ44ZU2T_cjs.createTool({
    id: "subagent",
...
${FORKED_SUBAGENT_TASK_NOTICE}`;
        ...
        subagentToRun = new chunkQJ3FDYCK_cjs.Agent({
        ...
        const allWorkspaceToolNames = workspace && allowedWs ? new Set(
          Object.keys(
            await chunkQJ3FDYCK_cjs.createWorkspaceTools(workspace, {
...
function patchTaskToolForFork(tool, toolId) {
  const stubExecute = async () => {
    ...
    if (toolId === "task_check") {
      const taskCheck = chunkQJ3FDYCK_cjs.summarizeTaskCheck([]);
      return {
        ...baseResult,
...
var AgentController = class {
  ...
  constructor(config) {
    ...
    this.#gatewayManager = new chunkOYARTFJQ_cjs.GatewayManager([...config.gateways ?? [], ...chunkOYARTFJQ_cjs.defaultGateways]);
    ...
    if (!defaultMode) {
      throw new Error(
        config.defaultModeId ? `Default mode not found: ${config.defaultModeId}` : "AgentController requires at least one agent mode"
      );
    }
  ...
  #wireSession(session) {
    ...
    session.thread.connect(this.createThreadDataStore(), session);
    session.setMachinery({
      getAgent: () => this.getCurrentAgent(session),
      subscribeToThread: ({ resourceId, threadId }) => this.getCurrentAgent(session).subscribeToThread({ resourceId, threadId }),
  ...
  async #createSessionForResource(ownerId, id, effectiveResourceId, tags, overrides) {
    const requestContext = overrides?.requestContext ?? new chunkPJIAL3WK_cjs.RequestContext();
    ...
    if (workspaceToConnect && workspaceToConnect instanceof chunkQJ3FDYCK_cjs.Workspace) {
      try {
        ...
        const initError = chunkXSOONORA_cjs.getErrorFromUnknown(error);
    ...
    if (tags && Object.keys(tags).length > 0) {
      ...
      const projectPath = this.config.initialState?.projectPath;
      if (projectPath) selectionTags.projectPath = projectPath;
    }
    const tagEntries = Object.entries(selectionTags);
    const threads = await session.thread.list();
    const candidates = tagEntries.length > 0 ? threads.filter((t) => {
   ...
   */
  async getSessionByResource(resourceId) {
  ...
  async resolveConfiguredMemory() {
    const configuredMemory = this.config.memory;
    if (!configuredMemory) return void 0;
    const memory = typeof configuredMemory === "function" ? await configuredMemory({ requestContext: new chunkPJIAL3WK_cjs.RequestContext(), mastra: this.getMastra() }) : configuredMemory;
    if (!memory) {
  ...
  async runInit() {
    if (this.config.storage && !this.#externalMastra) {
      ...
      this.#internalMastra = new chunkQJ3FDYCK_cjs.Mastra({
        logger: false,
    ...
    if (this.config.workspace && !this.workspaceInitialized && typeof this.workspace !== "function") {
      try {
        if (!this.workspace) {
          this.workspace = new chunkQJ3FDYCK_cjs.Workspace(this.config.workspace);
        }
  ...
  createThreadDataStore() {
    return {
      listThreads: ({ resourceId, includeForkedSubagents, metadata }) => this.queryThreads({ resourceId, includeForkedSubagents, metadata }),
      ...
      cloneThread: ({ sourceThreadId, resourceId, title, metadata }) => this.cloneThreadRow({ sourceThreadId, resourceId, title, metadata }),
  ...
  async writeThreadMetadataValue({
    threadId,
    key,
    value
  }) {
    ...
    try {
      ...
      if (thread) {
        await memoryStorage.saveThread({
          thread: { ...thread, metadata: { ...thread.metadata, [key]: value }, updatedAt: /* @__PURE__ */ new Date() }
        });
      }
    } catch {
    }
  ...
  async queryThreads({
    ...
    const threads = includeForkedSubagents ? result.threads : result.threads.filter((thread) => {
      const metadata2 = thread.metadata;
      return metadata2?.forkedSubagent !== true;
    });
  ...
  async queryThreadMessages({
    ...
    if (limit) {
      ...
      return result2.messages.map((msg) => this.convertToControllerMessage(msg)).reverse();
    }
  ...
  propagateRuntimeServicesToAgent(agent, _session) {
    ...
    if (this.browser && typeof agent.hasOwnBrowser === "function" && !agent.hasOwnBrowser() && typeof this.browser !== "function") {
  ...
  getAgentForMode(mode) {
    ...
    if (!this.#legacyAgentMode[mode.id]) {
      ...
      this.#legacyAgentMode[mode.id] = new chunkQJ3FDYCK_cjs.Agent({
        id: `${this.id}-agent`,
  ...
  async #dropSessionFromRegistry(resourceId, session) {
    const pending = this.#sessionsByResource.get(resourceId);
    if (!pending) return;
    const resolved = await pending.catch(() => void 0);
  ...
  resolveModeActiveTools(session) {
    ...
    const deniedTools = new Set(
      Object.entries(permissionRules.tools).filter(([, policy]) => policy === "deny").map(([tool]) => tool)
    );
    const deniedCategories = new Set(
      Object.entries(permissionRules.categories).filter(([, policy]) => policy === "deny").map(([cat]) => cat)
    );
...
` : "";
  ...
  convertToControllerMessage(msg) {
    ...
    if (msg.role === "signal") {
      const signal = chunkDC5C6QMM_cjs.mastraDBMessageToSignal(msg);
      if (signal.type === "user") {
  ...
  async buildToolsets(session, requestContext) {
    const builtInTools = {
      ask_user: chunkQJ3FDYCK_cjs.askUserTool,
      submit_plan: chunkQJ3FDYCK_cjs.submitPlanTool,
...
//# sourceMappingURL=chunk-NPAGJ3HR.cjs.map
//# sourceMappingURL=chunk-NPAGJ3HR.cjs.map