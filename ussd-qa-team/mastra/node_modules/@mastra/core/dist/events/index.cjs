...
var chunkPULYUCGC_cjs = require('../chunk-PULYUCGC.cjs');
var chunkWXNUQC46_cjs = require('../chunk-WXNUQC46.cjs');
...
// src/events/codec/registrations.ts
(() => {
  registerClass("DefaultGeneratedFile", {
    toData: (f) => ({ data: f.base64, mediaType: f.mediaType }),
    fromData: (d) => new chunkPUEQNTX6_cjs.DefaultGeneratedFile({ data: d.data, mediaType: d.mediaType })
  });
  registerClass("DefaultGeneratedFileWithType", {
    toData: (f) => ({ data: f.base64, mediaType: f.mediaType }),
    fromData: (d) => new chunkPUEQNTX6_cjs.DefaultGeneratedFileWithType({ data: d.data, mediaType: d.mediaType })
  });
  registerClass("DefaultStepResult", {
    ...
    fromData: (d) => new chunkWXNUQC46_cjs.DefaultStepResult(d)
  });
  return true;
})();

// src/events/codec/tags.ts
var CODEC_TAG = "__m_codec__";
var MAX_REGEXP_SOURCE_LENGTH = 1024;
function isEnvelope(value) {
  ...
  switch (tag) {
    ...
    case "Map":
    case "Set":
      return Array.isArray(value.v);
    case "Error":
      return typeof value.v === "object" && value.v !== null;
    case "Class":
...
function walk(v, seen) {
  ...
  if (t === "bigint") return { [CODEC_TAG]: "BigInt", v: v.toString() };
  if (t === "function" || t === "symbol") return void 0;
  if (v instanceof Date) {
    return { [CODEC_TAG]: "Date", v: v.toISOString() };
  }
  if (v instanceof RegExp) {
    return { [CODEC_TAG]: "RegExp", v: { source: v.source, flags: v.flags } };
  }
  if (v instanceof URL) {
    return { [CODEC_TAG]: "URL", v: v.toString() };
  }
  if (v instanceof Error) {
    return { [CODEC_TAG]: "Error", v: serializeError(v) };
  }
  if (v instanceof Map) {
    ...
    return { [CODEC_TAG]: "Map", v: entries };
  }
  if (v instanceof Set) {
    if (seen.has(v)) return null;
    seen.add(v);
  ...
  if (t === "object") {
    ...
    if (typeof maybeToJSON === "function") {
      return walk(maybeToJSON.call(v), seen);
    }
...
// src/events/unix-socket-pubsub.ts
var DEFAULT_MAX_REMOTE_CLIENT_QUEUED_BYTES = 64 * 1024 * 1024;
var MAX_LOCAL_REDELIVERIES = 6;
var REDELIVERY_DELAY_MS = 100;
...
var UnixSocketPubSub = class extends chunkMB3TIFKT_cjs.PubSub {
  socketPath;
  #server;
  #clientSocket;
  #isBroker = false;
  ...
  async close() {
    ...
    if (this.#isBroker) {
      await promises.unlink(this.socketPath).catch(() => {
      });
    }
    this.#isBroker = false;
  }
  ...
  async #start(forceReconnect) {
    ...
    this.#throwIfClosed();
    await promises.mkdir(path.dirname(this.socketPath), { recursive: true });
    this.#throwIfClosed();
    try {
      await this.#listen();
      this.#throwIfClosed();
  ...
  #listen() {
    return new Promise((resolve, reject) => {
      const server = net__default.default.createServer((socket) => this.#handleBrokerClient(socket));
      const onError = (error) => {
  ...
  #connectClient() {
    return new Promise((resolve, reject) => {
      const socket = net__default.default.createConnection(this.socketPath);
      const onError = (error) => {
  ...
  async #electBroker() {
    ...
    let lockFd;
    try {
      lockFd = await promises.open(lockPath, "wx");
    } catch (e) {
      if (e.code === "EEXIST") {
  ...
  async #isElectionLockStale(lockPath) {
    try {
      const lockStat = await promises.stat(lockPath);
  ...
  #handleBrokerClient(socket) {
    ...
    const write = client.writeChain.catch(() => {
    }).then(async () => {
      if (this.#brokerClients.get(client.socket) !== client || client.socket.destroyed) return;
  ...
  async #sendToBroker(frame) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        const code = error?.code;
        const transient = code === "EPIPE" || code === "ECONNRESET" || code === "ENOTCONN" || error?.message?.includes("socket closed before write completed") || error?.message?.includes("broker connection closed") || error?.message?.includes("not connected to a broker");
...
Object.defineProperty(exports, "CachingPubSub", {
  enumerable: true,
  get: function () { return chunkPULYUCGC_cjs.CachingPubSub; }
});
Object.defineProperty(exports, "withCaching", {
  enumerable: true,
  get: function () { return chunkPULYUCGC_cjs.withCaching; }
});
Object.defineProperty(exports, "EventEmitterPubSub", {
  enumerable: true,
  get: function () { return chunkMB3TIFKT_cjs.EventEmitterPubSub; }
});
Object.defineProperty(exports, "NoopLeaseProvider", {
  enumerable: true,
  get: function () { return chunkMB3TIFKT_cjs.NoopLeaseProvider; }
});
Object.defineProperty(exports, "PubSub", {
  enumerable: true,
  get: function () { return chunkMB3TIFKT_cjs.PubSub; }
});
Object.defineProperty(exports, "isLeaseProvider", {
  enumerable: true,
  get: function () { return chunkMB3TIFKT_cjs.isLeaseProvider; }
});
exports.MAX_LOCAL_REDELIVERIES = MAX_LOCAL_REDELIVERIES;
exports.UnixSocketPubSub = UnixSocketPubSub;
